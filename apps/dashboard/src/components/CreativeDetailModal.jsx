...
export function mapCreativeToModalData(creative) {
  ...
  return {
    ...
    model: meta.model || 'Flux Kontext Max',
    provider: meta.provider || 'Higgsfield',
    pose: meta.pose || '',
    scene: meta.scene || '',
    subject: meta.subject || (creative.show_model === false ? 'Product only' : 'On model'),
    bodyType: meta.body_type || '',
    framing: meta.framing || '',
    audience: meta.audience || '',
    catalogModel: meta.catalog_model || '',
    color: meta.color || '',
    seed: meta.seed || null,
...
export default function CreativeDetailModal({ data, onClose, onAction, onPrev, onNext }) {
  ...
  const configRows = [
    ...
    data.audience && { label: 'Audience', value: data.audience },
    data.catalogModel && { label: 'Catalog model', value: data.catalogModel },
    data.color && { label: 'Color variant', value: data.color },
