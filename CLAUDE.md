...
- Rate limiting via `lib/rate-limit.js` (Supabase-backed, async): generate 20/hr, video 10/hr, optimize 30/hr, import_reviews_csv 20/hr, generate_reviews 20/hr, public `review_submit:{ip}` 5/hr + `review_submit_global` 200/hr, `helpful_vote:{ip}` 30/hr + `helpful_vote_global` 500/hr
- Vercel 12-route limit: consolidated endpoints in `api/system.js` thin router (~114 lines) → 70 actions across 23 files in `lib/actions/`, dispatched by `?action=X` (GET) or `{ action }` body (POST). Errors are sanitized (strip API keys, DB strings) before returning to the client.
...
| `system.js` | Thin router (~114 lines) — delegates 70 actions to 23 files in `lib/actions/` |
