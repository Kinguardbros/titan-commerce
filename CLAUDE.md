...
- Vercel 12-route limit: consolidated endpoints in `api/system.js` thin router (~156 lines) → 74 actions across 25 files in `lib/actions/`, dispatched by `?action=X` (GET) or `{ action }` body (POST). Errors are sanitized (strip API keys, DB strings) before returning to the client.
- Error monitoring (P1-21, AUDIT-2026-08, Sentry replaced with DIY Telegram monitoring): `lib/notify.js` posts to a Telegram bot via plain `fetch` (no SDK), fail-open no-op unless `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` are set. `api/system.js`'s catch-all, `api/cron/detect-events.js`'s catch-all, `api/webhooks/shopify.js`'s handler-error catch, and `lib/actions/reviews-push.js`'s oversized-payload guard all call `captureException(err, { tags: {...} })` alongside the existing `console.error`. Uptime checked by a GitHub Actions cron (`.github/workflows/uptime.yml`, every 5 min, best-effort ~5-15 min actual) pinging `?action=health`, Telegram-alerting on non-200. See `Docs/RUNBOOK-monitoring.md`.
- Backup automation (P0-6, AUDIT-2026-08, fixed 2026-08-18 — the manual test-restore procedure never actually got run, so it's now automated instead of just documented): `.github/workflows/backup-daily.yml` runs `scripts/backup-database.mjs` daily (03:15 UTC) — shells out to system `pg_dump` against `SUPABASE_DB_URL` (Session Pooler URI, GitHub repo secret only, **not** a Vercel/app env var), dumps the `public` schema in custom format, excludes `pipeline_log` data (unbounded growth, not part of the restore-sanity contract), uploads as a `titan-db-dump` GH Actions artifact (30-day retention). `.github/workflows/test-restore-weekly.yml` runs Sundays 04:00 UTC — fetches the latest daily dump via `dawidd6/action-download-artifact` (cross-workflow artifact lookup `actions/download-artifact@v4` can't do), restores it into an ephemeral `postgres:16` service container (primed first via `scripts/ci-prime-restore-roles.sql` with stub `authenticated`/`anon`/`service_role` roles + a stub `auth` schema — the RLS policies in `sql/*.sql` reference these and a vanilla Postgres image has neither), then runs `scripts/restore-sanity.mjs` (5 row-count checks: stores/users/products/product_reviews/schema_migrations). Both workflows Telegram-alert `if: failure()` only — silent on success, same contract as `uptime.yml`. See `Docs/RUNBOOK-backup-restore.md` section 0 for the full writeup + Dan's one-time `SUPABASE_DB_URL` secret setup.
...
| `Settings.jsx` | Settings tab shell — visible to every logged-in user (P1-14). Always renders `ChangePasswordForm` (skipped for master, no backing `users` row); `UsersManager` (admin user table) and the **Maintenance** section (2026-08-18 — "Cleanup orphan photos" button → `cleanup_orphan_review_photos`, toast result) are both gated inside on `user?.role === 'admin'`. |
...
| `notify.js` | `initSentry()` (no-op, kept for call-site compat) / `captureException(err, { tags })` — posts to a Telegram bot via plain `fetch`, fail-open no-op unless `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` are set (P1-21, AUDIT-2026-08, replaced Sentry with DIY Telegram monitoring). Called from `api/system.js`, `api/cron/detect-events.js`, `api/webhooks/shopify.js`, `lib/actions/reviews-push.js`. |
...
| `products` | Shopify products (synced): handle, title, price, images JSONB, `tags` JSONB (= collection memberships, set by full sync only), `product_type`, `vendor`, `status` (active/archived), `cogs` (flat per-product COGS), `variant_cogs` (JSONB per-variant COGS map — was live-only drift until captured by `sql/fix-products-cogs-drift.sql`), `has_size_chart`, `garment_length` (text, `'short' \| 'mid' \| 'long' \| NULL` — populated lazily by Claude Vision on the first v1 generation, cached forever), `store_id` FK, `publication_online_store` (BOOLEAN, cached Online Store publication state; false = unlisted) |
| `creatives` | Generated ad creatives (image/video): file_url, storage_path, format, status (generating/**polling**/pending/approved/rejected/published/failed — `polling` added P1-20, AUDIT-2026-08, atomic claim state between `generating` and finalization), `polling_started_at` (TIMESTAMPTZ, set when a row is claimed, used by `cleanup_stale` to detect an orphaned claim), `metadata` JSONB (style, audience, model, hook_used, poll info), `product_id`, `story_id`, `store_id` FK, `deleted_at` (TIMESTAMPTZ, nullable — soft-archive marker, Phase A of a two-phase `store_id` `NOT NULL` hardening plan; rows with `deleted_at IS NOT NULL` are excluded from `api/creatives/list.js` and `cleanup_stale`'s stale-pending sweep. `sql/archive-null-store-id-creatives.sql` archived the legacy `store_id IS NULL` rows on 2026-08-18 — 0 rows matched at apply time, so this is forward-looking infra rather than an active cleanup. Phase B, the `store_id` `NOT NULL` constraint itself, is deferred pending an observation period on this change). Migration `sql/add-creatives-polling-status.sql`, `sql/archive-null-store-id-creatives.sql`. |
...
| `users` | Per-user auth: `username` UNIQUE, `password_hash` (scrypt via `lib/password.js`), `role` CHECK IN ('admin','member'), `permissions` TEXT[] (closed set from `lib/permissions.js`), `store_access` UUID[] (which stores user sees), `active` BOOL, `full_name`, `email` (optional), `last_login`, `token_version` (INT, default `1` — bumped on password change/reset to invalidate outstanding sessions, P1-15), `must_change_password` (BOOL, default `false` — set `true` by admin `reset_password`, forces the forced-change screen on next login, P1-14), `api_token` (TEXT UNIQUE, legacy plaintext bearer token — `sql/add-user-api-token.sql`; being phased out), `api_token_hash` (TEXT, SHA-256 hex of the bearer token — P2 AUDIT-2026-08, `sql/add-users-api-token-hash.sql`, see Bearer `api_token` note above). Admin trumps permissions/store_access. See RBAC section below. Migrations: `sql/add-users-and-permissions.sql` → `sql/add-user-token-version.sql` → `sql/add-user-api-token.sql` → `sql/add-users-api-token-hash.sql`. |
| `oauth_nonces` | Single-use, TTL'd nonces for the Shopify OAuth callback's `state` param (P2, AUDIT-2026-08, fixed 2026-08-17) — `nonce` (TEXT PK), `store_id` FK (`ON DELETE CASCADE`), `created_at`, `expires_at` (10 min from issue). Row inserted at CONNECT, looked up + deleted at CALLBACK regardless of outcome. See `api/auth/shopify.js` note above. Migration `sql/add-oauth-nonces-table.sql`. |
...
- **Migration verification (C1-C3, AUDIT-2026-08-B, fixed 2026-08-19):** a registered row does NOT prove the migration ran — the 2026-08-17 backfill registered every file unchecked, which let `add-rate-limits.sql` sit "applied" while the table didn't exist and every rate limiter silently fail-opened in prod. Checklist for EVERY applied migration: (1) apply the file, (2) run `SUPABASE_ACCESS_TOKEN=sbp_... node scripts/verify-migrations.mjs` — it checks every registered file's primary objects against live catalogs, stamps `schema_migrations.verified_at` on confirmed rows, exits 1 on GHOST/PARTIAL drift. Needs live DB access (Management API), so it is deliberately NOT in CI — the applier runs it as the last step. `schema_migrations` itself now has RLS on + anon/authenticated revoked (`sql/harden-schema-migrations.sql` — it was the only internet-writable table in public). `lib/rate-limit.js` escalates a missing `rate_limits` table (Postgres `42P01`) to Telegram via `captureException` while still failing open, so this class of ghost can't hide again. Superseded files carry `-- DEPRECATED` as their FIRST line (P1-18 style); `sql/fix-creatives-style-check.sql` is the single source of truth for `creatives_style_check` (extend in place for new styles — supersedes `add-static-styles.sql`/`relax-style-check.sql`), and `sql/fix-products-cogs-drift.sql` reconciled the `products.cogs`/`variant_cogs` file-vs-live drift.
...
- `sql/add-oauth-nonces-table.sql` (P2, AUDIT-2026-08, applied 2026-08-17) — creates `oauth_nonces` (nonce PK, store_id FK, expires_at), RLS enabled no policies (service-role only). See `api/auth/shopify.js` note above.
- `sql/add-users-api-token-hash.sql` (P2, AUDIT-2026-08, applied 2026-08-17) — adds `users.api_token_hash`, backfills from every existing plaintext `api_token` (1 row on prod at apply time). See Bearer `api_token` note above.
- `sql/archive-null-store-id-creatives.sql` (Phase A soft-archive, applied 2026-08-18) — adds `creatives.deleted_at` (`TIMESTAMPTZ`, nullable) + `idx_creatives_deleted_at`, marks legacy `store_id IS NULL` rows archived. 0 rows matched at apply time (`sql/add-stores.sql` had already backfilled every `creatives` row to Elegance House during the original multi-store migration) — additive infra for the deferred Phase B (`store_id` `NOT NULL` constraint), not an active cleanup. Fully reversible: `UPDATE creatives SET deleted_at = NULL WHERE ...`. See `creatives` table note above.
...
TELEGRAM_BOT_TOKEN=             # Required for error notifications (P1-21, AUDIT-2026-08 — replaces SENTRY_DSN). EMPTY = lib/notify.js's captureException() is a no-op, existing console.error behavior unchanged. See Docs/RUNBOOK-monitoring.md for BotFather setup. Also set as a GitHub repo secret (same name) for .github/workflows/uptime.yml.
TELEGRAM_CHAT_ID=               # Required for error notifications, paired with TELEGRAM_BOT_TOKEN above. Also set as a GitHub repo secret (same name).
...
- **Alerting**: a successful login via `APP_PASSWORD` should ideally trigger a Telegram notification (via `lib/notify.js`), so master-fallback usage is visible instead of silent. Not wired yet — follow-up, post-P1-21.
...
- Backend error monitoring is Telegram-based, no dependency: `lib/notify.js` posts via plain `fetch` (P1-21, AUDIT-2026-08 — `@sentry/node` was removed when Sentry was replaced with DIY Telegram monitoring per Dan's zero-cost preference). Fail-open unless `TELEGRAM_BOT_TOKEN` + `TELEGRAM_CHAT_ID` are set.
...
- **Session signature comparison (P2, AUDIT-2026-08, fixed 2026-08-17):** `verifyAuth`'s HMAC signature check uses a `safeEqual(a, b)` helper (length check, then `crypto.timingSafeEqual`) instead of `!==` — was the one comparison of a secret value in `lib/auth.js` not using the codebase's `timingSafeEqual` discipline (`password.js`, `webhooks/shopify.js` already did). Same helper backs the `api_token_hash` compare above.
...
- **Bearer `api_token` (userscript/API access):** `lib/auth.js`'s `verifyAuth` also accepts a flat 64-char hex token (no `.` — session tokens are `base64.hexsig` and always contain one). Generated via `generate_api_token` action (admin-only, `admin:users` perm), one-time reveal in Settings > Users. Used by `scripts/titan-amazon-userscript.user.js` to call `import_amazon_reviews` from the Amazon page. Regenerating overwrites the previous token (old one stops working immediately). Never replaces the session-token dashboard login path — fully parallel. Not subject to the `tv` check (separate revocation model: regenerating overwrites the old token).
  - **SHA-256 hash storage (P2, AUDIT-2026-08, fixed 2026-08-17):** `users.api_token` was plaintext (I-2 tech debt). `generate_api_token` now writes only `users.api_token_hash` (SHA-256 hex) and clears `api_token` to `NULL` — the plaintext token is returned once in the API response and never persisted. `verifyApiToken` looks up by `api_token_hash` first (constant-time hash compare via `safeEqual`, same helper used for the session-signature check below), falling back to the legacy plaintext `api_token` column only when `api_token_hash IS NULL` for that row (`console.warn('[auth] plaintext api_token match — user {id} needs to regenerate')` — a data-integrity guard rejects the fallback if `api_token_hash` is set but didn't match, rather than trusting a stale plaintext value). Migration `sql/add-users-api-token-hash.sql` adds the column + backfills every existing plaintext token's hash (dual-mode — no forced regeneration). Follow-up: drop the plaintext `api_token` column once logs show zero fallback warnings for a full burn-in cycle.
...
- No dedicated Log Drain on Hobby — Telegram error notifications (`lib/notify.js`, P1-21, AUDIT-2026-08) compensate for structured error visibility; there's still no first-class request/access log export.
...
| 🟢 LOW | `users.api_token` plaintext column + `lib/auth.js` plaintext fallback (P2, AUDIT-2026-08) | Burn-in window for the `api_token_hash` SHA-256 migration (`sql/add-users-api-token-hash.sql`). Drop the plaintext column + the fallback branch in `verifyApiToken` once logs show zero `[auth] plaintext api_token match` warnings for a full deploy cycle. |
...
SUPABASE_ACCESS_TOKEN=sbp_... node scripts/verify-migrations.mjs  # MANDATORY after every apply — verifies ledger vs live, stamps verified_at

# Tests
npm test                             # Vitest — auth (incl. tv/session-revocation, self-service password change, OAuth HMAC fail-closed + nonce single-use, api_token hash lookup + plaintext fallback), rate-limit, profit, routing, CSV parser, image validation, import-guard, contracts, migration tracking, atomic creative-poll claim, monitoring/health + Telegram notify, review source CHECK contract, per-store CORS resolution, parallel cron store processing, pipeline_log user/cron/webhook attribution (531 tests)
```
