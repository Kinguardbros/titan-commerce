...
- Vercel 12-route limit: consolidated endpoints in `api/system.js` thin router (~114 lines) → 67 actions across 22 files in `lib/actions/`, dispatched by `?action=X` (GET) or `{ action }` body (POST). Errors are sanitized (strip API keys, DB strings) before returning to the client.
...
| `product_reviews` | Per-product reviews: author, rating (1–5 CHECK), title, body, `photo_url`, `verified`, `review_date` (NOT NULL, default today), `source` (manual/csv/ai, CHECK), `status` (pending/approved/published/rejected, CHECK), `dirty` (published+edited → awaits re-push), `store_id` + `product_id` FK (product CASCADE). Unique dedup index `(store_id, product_id, author, md5(body))`. RLS: authenticated SELECT, service-role writes. Hardening migration: `sql/add-product-reviews-hardening.sql`. |
