...
- Rate limiting via `lib/rate-limit.js` (Supabase-backed, async): generate 20/hr, video 10/hr, optimize 30/hr, import_reviews_csv 20/hr, generate_reviews 20/hr, public `review_submit:{ip}` 5/hr + `review_submit_global` 200/hr
- Vercel 12-route limit: consolidated endpoints in `api/system.js` thin router (~114 lines) → 68 actions across 23 files in `lib/actions/`, dispatched by `?action=X` (GET) or `{ action }` body (POST). Errors are sanitized (strip API keys, DB strings) before returning to the client.
...
| `product_reviews` | Per-product reviews: author, rating (1–5 CHECK), title, body, `photo_url`, `verified`, `review_date` (NOT NULL, default today), `source` (manual/csv/ai/web, CHECK), `status` (pending/approved/published/rejected, CHECK), `dirty` (published+edited → awaits re-push), `email` (optional, from web submissions, never shown on site), `store_id` + `product_id` FK (product CASCADE). Unique dedup index `(store_id, product_id, author, md5(body))`. RLS: authenticated SELECT, service-role writes. Migrations (run **in order**): `sql/add-product-reviews.sql` (base table) → `sql/add-product-reviews-hardening.sql` (dedup index, review_date NOT NULL, status/source CHECK — note: this sets source CHECK to manual/csv/ai only) → `sql/add-review-email.sql` (adds `email` + extends source CHECK with `'web'`). Running hardening without the email migration → `source='web'` inserts fail the CHECK. |
...
SITE_URL=***
STOREFRONT_URL=***              # CORS allow-origin(s) for public review submit, comma-separated (e.g. https://isolaswim.com,https://swimwear-brand.myshopify.com). Unset → hardcoded fallback in api/system.js
