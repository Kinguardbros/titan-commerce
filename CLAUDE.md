...
- Vercel 12-route limit: consolidated endpoints in `api/system.js` thin router (~114 lines) → 71 actions across 23 files in `lib/actions/`, dispatched by `?action=X` (GET) or `{ action }` body (POST). Errors are sanitized (strip API keys, DB strings) before returning to the client.
...
| `system.js` | Thin router (~114 lines) — delegates 71 actions to 23 files in `lib/actions/` |
