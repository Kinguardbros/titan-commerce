...
She is barefoot on a quiet sunny beach, clear blue sky, bright beautiful sunny day. The model is FACING the sun — the sun (and the camera) are in front of her, so her face, body and the swimsuit are lit directly head-on. She is NOT backlit, the sun is NOT behind her. Bright direct daylight on the front of the model. Clean neutral white balance. NOT overcast, NOT cloudy, NOT grey sky, NOT golden hour, NOT sunset.

PRODUCT LIGHTING (critical — this is the most important instruction): The light source is DIRECTLY IN FRONT of the model, pointed straight at her — like a photographer holding a big bright studio softbox right at her, or like she is standing facing the midday sun. The light hits the swimsuit FULL FRONT, head-on. The SWIMSUIT must be the BRIGHTEST, most evenly lit object in the entire image — slightly OVEREXPOSED toward the garment so it is unmistakably bright, never dark, never dim. ZERO shadows on the swimsuit: no shadow under the bust, no shadow at the waistband, no shadow in the folds or seams, no dark side of the garment, no cast shadow from her arms onto the garment, no part of the swimsuit falling into shade. Every part of the swimsuit — fabric texture, exact color and pattern, trims, stitching, seams, waistband — is crisply lit, bright, and fully readable. The garment is NOT in the model's own shadow. Expose for the product first: if the swimsuit looks dark, dim, or has ANY shadow on it, the lighting is WRONG and must be brighter and more frontal.

Product: ${product.title}

${poseAndFraming}

Garment: High-waisted bottoms cover navel. Fabric smooth, zero bunching. Match reference exactly. The garment is fully lit — no shadows anywhere on it.
...
LIGHTING CHECK — READ LAST AND OBEY: The light comes from DIRECTLY IN FRONT of the model (sun or studio softbox in front of her, not behind, not to the side). The swimsuit is hit full-front, head-on. The swimsuit is the BRIGHTEST thing in the image — bright, clearly exposed, slightly overexposed toward the garment, NEVER dark or dim, with ZERO shadows anywhere on it (not under the bust, not at the waist, not in folds, not from her arms). If the swimsuit is dark, dim, or has any shadow on it, the result is WRONG.

NEGATIVE: ${catalogNegativePrefix}dark swimsuit, dim swimsuit, underexposed garment, swimsuit in shadow, shadow under the bust, shadow at the waistband, shadow in the folds, arm casting shadow on the garment, dark areas on the garment, garment in shade, half-lit product, underlit swimsuit, model's own shadow on the garment, side lighting, rim lighting only, backlit, sun behind the model, overcast sky, grey clouds, cloudy weather, dark photo, underexposed, moody lighting, harsh shadows, dramatic lighting, golden hour, sunset, orange tones, plastic skin, porcelain smoothing, AI face, blurry face, smooth featureless skin, doll eyes, slim body, flat stomach, thigh gap, text, watermarks${framingNegative}.`.trim();
