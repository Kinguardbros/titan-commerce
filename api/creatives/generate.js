...
NEGATIVE: No plastic skin, no porcelain smoothing, no fitness model body, no slim body, no flat stomach, no toned arms, no thigh gap, no exaggerated curves, no sexual posing, no duck face, no visible logos or text, no watermarks, no oversaturated colors, no glossy wet-look skin, no extra fingers, no distorted hands, no skinny model, no athletic build.`.trim();
    ...
    if ((isProductCatalog || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7)) {
      const garmentOverride = isNonSwimGarment
        ? `\n\n━━━━━━━━━━━━━━━━━━━━━━━━\nGARMENT TYPE OVERRIDE (CRITICAL — READ FIRST): This product is a ${garmentDescriptor.toUpperCase()}, NOT a swimsuit, NOT a bikini. The garment in the reference image is a ${garmentDescriptor} — render the GARMENT itself EXACTLY as shown in the reference (same construction, same length, same coverage, same silhouette, same neckline, same sleeves/straps if any). IGNORE any references in the prompt to "swimsuit", "bikini", "one-piece", "waistband sitting at the waist", "high-waist tummy control", "navel coverage", or other swimwear-specific instructions — they DO NOT apply to this ${garmentDescriptor}.\n\nIMPORTANT — FRAMING/CROP RULES: This override controls the GARMENT TYPE only. The image FRAMING, CROP, COMPOSITION, and BACKGROUND must follow the framing instructions specified later in the prompt (vertical 4:5, model positioning, framing crop). Do NOT change the framing to fit the entire garment if the prompt specifies a cropped close-up or a specific framing. If a maxi dress would not fit the specified frame, that is OK — render the dress as it is, but cropped per the prompt's framing rules.\n━━━━━━━━━━━━━━━━━━━━━━━━`
        : `\n\n━━━━━━━━━━━━━━━━━━━━━━━━\nGARMENT FIDELITY (CRITICAL — READ FIRST): The product reference image is the ULTIMATE SOURCE OF TRUTH for the GARMENT TYPE only. Render the garment EXACTLY as shown in the reference — same type (one-piece / bikini set / tankini / dress / cover-up / etc), same length, same coverage, same silhouette, same neckline depth, same strap style, same trim, same construction, same color and pattern. If the prompt below mentions "swimsuit" or "bikini" generically, that is just the product category — the SPECIFIC garment to render comes from the reference image. If the reference shows a long flowing dress, render a long flowing dress (do NOT default to bikini).\n\nIMPORTANT — FRAMING/CROP RULES: This override controls the GARMENT TYPE only. The image FRAMING, CROP, COMPOSITION, and BACKGROUND must follow the framing instructions specified later in the prompt (vertical 4:5, model positioning, framing crop). Do NOT change the framing to fit the entire garment if the prompt specifies a cropped framing. The garment may be cropped by the frame — that is correct.\n━━━━━━━━━━━━━━━━━━━━━━━━`;
