...
import { V9_PROMPT_BODY } from '../../lib/v9-prompt.js';
...
async function handler(req, res) {
  ...
  try {
    ...
    const isProductCatalogV9 = style === 'product_catalog_v9';
    const isProductCatalogV10 = style === 'product_catalog_v10';
    ...
    if (audience || isProductCatalog || isRealisticBeach || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7 || isProductCatalogV8 || isProductCatalogV9 || isProductCatalogV10) {
      ...
      if (variant_image_url && (isProductCatalog || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7 || isProductCatalogV8 || isProductCatalogV9 || isProductCatalogV10)) {
    ...
    if (isProductCatalog) {
      ...
      const modelDesc = modelDescMatch ? modelDescMatch[1].trim() : 'Real plus-size woman, US size 14-16, late 30s to mid 40s. Genuinely curvy body — soft natural belly, fuller hips and thighs that touch, soft upper arms with natural fullness, natural bust. NOT slim, NOT athletic, NOT a fitness body. She carries real weight in the midsection, hips, and thighs like a typical mother of two in her 40s — but she looks visibly SHAPED and SUPPORTED by the swimsuit (the garment smooths her tummy, lifts her bust, and contours her waist so the silhouette reads SLIMMER than her real body would alone — that flattering shaping effect IS the product). Warm relatable expression with a soft natural smile. Natural windswept hair, minimal makeup, no jewelry, no tattoos.';
      ...
      void framingText;
      // Framing block borrowed from the v7/v8 'ALWAYS / NEVER' wording — that phrasing
      // triggers Nano Banana to respect the crop on the first try far more reliably than
      // 'feet NOT visible' negation. Post-process crop is still applied as a safety net.
      const framingBlock = `\n\n━━━━━━━━━━━━━━━━━━━━━━━━\n=== COMPOSITION & FRAMING ===\nVertical 4:5. Subject positioned center frame horizontally, occupying the central 80-90 percent of the vertical frame. Generous negative space above the head (sky, soft background) for potential text overlay. Frame ALWAYS extends from the top of the head down through MID-CALF ONLY — this is a fixed crop regardless of garment length. The feet and ankles are NOT in the frame. The bottom edge of the photo sits at the middle of her lower leg, between the knee and the ankle. NEVER widen the frame to fit the feet. NEVER zoom out. The frame stays mid-calf ALWAYS.\n━━━━━━━━━━━━━━━━━━━━━━━━`;
      const framingNegative = ', head cropped, top of head cut off, full body shot, visible feet, visible ankles, feet in frame, ankles in frame, sand at the feet, head-to-toe shot, full-length portrait, model standing in full body view';
...
LIGHT ON THE MODEL AND GARMENT — FRONTAL SOFTBOX, EVEN COVERAGE TOP-TO-BOTTOM:
The model and the garment are lit by a BIG SOFTBOX positioned IN FRONT of the model at her chest height, PLUS soft fill light on BOTH SIDES. This is a commercial e-commerce catalog lighting setup — NOT natural sunlight, NOT midday sun, NOT a top-down light source. The light falls EVENLY across the entire body from head to mid-thigh: the chest, waist, hips, and skirt/bottoms are ALL exposed at the SAME brightness. There is NO top-down fall-off, NO shadow gradient where the lower half is darker than the top, NO "natural sun from above" character. The lower half (waistband, skirt, bottoms, hips) reads JUST AS BRIGHT as the upper half (face, chest, top) — they are uniformly lit. Only the gentlest contact shadows are allowed (a soft shadow under the chin, a tiny shadow tucked behind an arm). NO hard cast shadow stretching off to one side, NO side-lit shadow on the garment, NO dark side of the body. The catalog look is FLAT, EVEN, COMMERCIAL — like a studio softbox setup brought outdoors onto a beach backdrop.

EXPOSURE: the MODEL and GARMENT are well-exposed across their entire vertical extent — clearly readable, never dim, never dark, never moody, but ALSO never overexposed, never blown out, never washed out — a natural, balanced, true-to-life exposure. The lower half of the garment is exposed identically to the upper half — NOT one stop darker, NOT in shadow, NOT in falloff. Black fabric reads as a rich dark grey-black with the ribbed texture / pleating / seams clearly visible — NOT crushed to a flat black silhouette. The BACKGROUND is also properly exposed — visible sea, waves, sand, dune grass, and a blue sky with clouds, all holding full detail — NOT blown out to pure white, NOT vaporised, NOT a foggy haze.

THE GARMENT: the GARMENT is the hero of this photo. It is lit by the frontal softbox and is well-exposed and clearly readable — never dim, never grey-flat, but also never blown out: fabric texture${colorFromReference ? ", exact color and pattern" : ""}, ribbing/pleating, trims, stitching, seams, waistband all crisply visible. Black fabric reads as a rich dark grey-black with all the ribbed / pleated texture catching the light — NOT crushed to a flat black silhouette, NOT a washed-out grey. Exposed neutrally — natural and true to life, the shadows on the fabric just gently filled so the deepest folds and the underside of the bust stay readable. CRITICAL: the LOWER HALF of the garment (briefs / bottoms / skirt / waistband) is lit AT EXACTLY THE SAME BRIGHTNESS as the upper half (top / bra / chest) — it does NOT fall darker, it does NOT sink into shadow, it does NOT show top-down sunlight falloff. The garment reads as a single uniformly-lit surface from chest to mid-thigh. ZERO hard shadows on the garment. (This applies to the GARMENT and model — it does NOT change the scene: the sky stays a bright blue with soft clouds, the background stays a properly-exposed real beach with full visible detail, not blown out to white.) If the lower half of the garment is visibly darker than the upper half, OR a hard directional / side-lit shadow appears on the body / garment / sand, OR the background / model / sand / sky is overexposed and washed out to white, OR the background is a gloomy dark grey, the result is WRONG.

GRADE: clean, natural, e-commerce catalog — true-to-life colors with full saturation and full tonal range, neutral skin tone. NOT a cool / grey / blue grade, NOT a heavy orange filter, NOT golden-hour warm, NOT washed-out, NOT overexposed, NOT a bright hazy wash, NOT flat lifeless lighting.
━━━━━━━━━━━━━━━━━━━━━━━━
...
LIGHTING — READ THIS: FRONTAL SOFTBOX lighting on the model and garment (commercial e-commerce catalog setup, NOT natural midday sun) — the body is lit EVENLY from chest to mid-thigh, the lower half of the garment is JUST AS BRIGHT as the upper half, NO top-down sun falloff, NO shadow gradient where bottoms/skirt are darker than the top. Well-exposed and clearly readable. Only soft contact shadows under the chin / behind an arm. NO hard side-lit / directional shadow on the product, body, or sand. Bright BLUE sky with a few soft white clouds, light haze at the horizon, background holds full visible detail (NOT blown out to white, sand and sky NOT vaporised). Clean neutral grade — NOT cool/grey, NOT a heavy orange filter, NOT golden hour, NOT washed-out, NOT moody. Black fabric shows texture, not crushed black.
${framingBlock}

${catalogFinalCheck}

NEGATIVE: ${catalogNegativePrefix}${catalogHighWaist ? 'visible belly button, exposed navel, partially visible navel, peek of belly button, navel showing above the waistband, gap above the waistband, low-set waistband, bare midriff, low-rise bottoms, mid-rise bottoms, low-waist cut, exposed stomach, ' : ''}lower half of garment darker than upper half, skirt or bottoms in shadow, waistband sinking into darkness, top-down sunlight falloff, brightness gradient from top to bottom, harsh midday sun character, golden-hour warm cast on the body, blown-out white background, featureless white background, empty white background, foggy haze, missing background, studio backdrop, no beach visible, overexposed background, white void behind the model, heavy grey overcast, gloomy dark sky, directional shadow, hard cast shadow, side lighting, side-angle sun, shadow on the sand to one side, dark side of the body, shadow on one leg, shadow under the bust, deep shadows on the swimsuit, dark areas on the garment, swimsuit lost in shadow, underlit swimsuit, crushed blacks, garment crushed to pure black, dramatic lighting, moody lighting, dim, dark photo, underexposed, heavy orange filter, washed-out colors, flat lifeless lighting, cool blue grade, overexposed, overexposed model, blown-out highlights, blown-out skin, blown-out sky, blown-out sand, washed-out face, washed-out background, bright hazy wash, vaporised background, white-washed scene, milky overexposure, plastic skin, porcelain smoothing, AI face, blurry face, smooth featureless skin, doll eyes, slim body, athletic body, fitness model body, flat athletic stomach, thigh gap, skinny model, model body proportions, swimsuit cutting into flesh, fabric digging into soft tissue, muffin top above waistband, skin spillover around bottoms, visible fabric stress lines pulling on the garment, swimsuit pulling tight asymmetrically, bunching fabric, baggy ill-fitting swimsuit, sagging bust, flattened bust, low-angle shot, shot from below, worm's-eye view, upward camera angle, distorted perspective, foreshortened legs, text, watermarks${framingNegative}.`.trim();
...
NEGATIVE: beach, ocean, sand, water, sky, outdoor, nature, sunset, golden hour, props, furniture, floor line, horizon line, gradient backdrop, colored background, dark background, shadow on the wall, harsh shadow, hard cast shadow, side lighting, directional shadow, dark side of the body, dim, dark photo, underexposed, overexposed, blown-out highlights, washed out, hazy bright wash, crushed blacks, garment crushed to pure black, deep shadows on the swimsuit, dark areas on the garment, ${catalogHighWaist ? 'visible belly button, exposed navel, partially visible navel, peek of belly button, navel showing above the waistband, gap above the waistband, low-set waistband, bare midriff, low-rise bottoms, mid-rise bottoms, low-waist cut, exposed stomach, ' : 'visible belly button, exposed navel, low-rise bottoms, mid-rise bottoms, '}plastic skin, porcelain smoothing, AI face, blurry face, smooth featureless skin, doll eyes, slim body, flat stomach, thigh gap, low-angle shot, shot from below, distorted perspective, text, watermarks.`.trim();
    ...
    } else if (isProductCatalogV9) {
      // Product Catalog v9 — clean monolithic prompt rebuild of v1. Same bright sunny beach
      // look as v1 but uses the v7/v8 prompt structure (single monolithic body, strict
      // sections, ALWAYS/NEVER framing wording) that consistently produces 3/4-body crops
      // on Nano Banana's first try. Avatar reference is REQUIRED (frontend guard) — no
      // text modelDesc fallback like v1 had.
      const v9Prefix = `REFERENCE IMAGES: image 1 AND the last image = THE MODEL (the SAME woman, shown twice — use her exact face, hair, skin tone, body shape, and age). Any image in between = THE GARMENT (cropped product shots — copy the swimsuit's color, cut, neckline, strap style, fabric texture, seaming, construction, coverage exactly; do NOT let it influence the model's face).\n\nProduct: ${product.title}\n\n`;
      const v9HighWaistBlock = catalogHighWaist
        ? `\n\n━━━━━━━━━━━━━━━━━━━━━━━━\n=== HIGH-WAIST TUMMY-CONTROL — MANDATORY, READ TWICE ===\nThis swimsuit is TUMMY CONTROL. The bottoms / one-piece waistline sits VERY HIGH — at the natural waist, WELL ABOVE the belly button. CRITICAL: the waistband sits NOTICEABLY HIGHER than it appears in the product reference photo — raise it up so the top edge reaches the natural waist / just below the bottom of the rib cage. The navel is buried several centimetres BELOW the top edge of the fabric, fully covered. The belly button is COMPLETELY, ENTIRELY hidden — not a peek, not a sliver, not partially — there is NO gap, NO cutout, NO bare skin between the bra/top and the high waistband where the navel could show. The fabric covers the entire stomach from the natural waist down, hugging and smoothing it. This is a FULL high-rise brief, NOT a mid-rise, NOT a low-rise. If you see ANY skin of the navel area above the waistband, the waistband is too low — raise it higher until the navel is fully hidden.\n━━━━━━━━━━━━━━━━━━━━━━━━`
        : '';
      prompt = `${v9Prefix}${V9_PROMPT_BODY}${v9HighWaistBlock}`;
    } else if (isProductCatalogV10) {
      // Product Catalog v10 — clone of v9 with v1's verbatim LIGHTING block + GARMENT-specific
      // lighting paragraph swapped in. User wanted v9 but with v1's longer, more explicit
      // lighting wording (FRONTAL SOFTBOX + SIDE FILL + GARMENT paragraph with fabric/ribbing/
      // seams/no-crushed-black details).
      const v10Prefix = `REFERENCE IMAGES: image 1 AND the last image = THE MODEL (the SAME woman, shown twice — use her exact face, hair, skin tone, body shape, and age). Any image in between = THE GARMENT (cropped product shots — copy the swimsuit's color, cut, neckline, strap style, fabric texture, seaming, construction, coverage exactly; do NOT let it influence the model's face).\n\nProduct: ${product.title}\n\n`;
      const v10HighWaistBlock = catalogHighWaist
        ? `\n\n━━━━━━━━━━━━━━━━━━━━━━━━\n=== HIGH-WAIST TUMMY-CONTROL — MANDATORY, READ TWICE ===\nThis swimsuit is TUMMY CONTROL. The bottoms / one-piece waistline sits VERY HIGH — at the natural waist, WELL ABOVE the belly button. CRITICAL: the waistband sits NOTICEABLY HIGHER than it appears in the product reference photo — raise it up so the top edge reaches the natural waist / just below the bottom of the rib cage. The navel is buried several centimetres BELOW the top edge of the fabric, fully covered. The belly button is COMPLETELY, ENTIRELY hidden — not a peek, not a sliver, not partially — there is NO gap, NO cutout, NO bare skin between the bra/top and the high waistband where the navel could show. The fabric covers the entire stomach from the natural waist down, hugging and smoothing it. This is a FULL high-rise brief, NOT a mid-rise, NOT a low-rise. If you see ANY skin of the navel area above the waistband, the waistband is too low — raise it higher until the navel is fully hidden.\n━━━━━━━━━━━━━━━━━━━━━━━━`
        : '';
      prompt = `${v10Prefix}${V10_PROMPT_BODY}${v10HighWaistBlock}`;
...
NEGATIVE: No plastic skin, no porcelain smoothing, no fitness model body, no slim body, no flat stomach, no toned arms, no thigh gap, no exaggerated curves, no sexual posing, no duck face, no visible logos or text, no watermarks, no oversaturated colors, no glossy wet-look skin, no extra fingers, no distorted hands, no skinny model, no athletic build.`.trim();
    ...
    if ((isProductCatalog || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7 || isProductCatalogV8 || isProductCatalogV9 || isProductCatalogV10)) {
    ...
    if ((isProductCatalog || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7 || isProductCatalogV8 || isProductCatalogV9 || isProductCatalogV10) && product_color) {
    ...
    if ((isProductCatalog || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7 || isProductCatalogV8 || isProductCatalogV9 || isProductCatalogV10) && show_model !== false) {
    ...
    if (ai_model === 'fal_nano_banana' || ai_model === 'fal_nano_banana_pro') {
      ...
      if (reference_url || images.length > 0) {
        ...
        const outAspectRatio = (isProductCatalog || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7 || isProductCatalogV8 || isProductCatalogV9 || isProductCatalogV10) ? '4:5'
          : aspect_ratio;
        ...
        const falPrompt = (isProductCatalog || isProductCatalogV2 || isProductCatalogV3 || isProductCatalogV4 || isProductCatalogV5 || isProductCatalogV6 || isProductCatalogV7 || isProductCatalogV8 || isProductCatalogV9 || isProductCatalogV10)
