...
export async function detectEventsForStore(storeId, topProducts, supabase) {
  ...
  // topProducts only includes products with recent sales; new/unsold products
  // would never get checked without this second pass.
  ...
  // Batch: fetch all product_ids that have at least 1 non-failed creative
  const allPids = (allStoreProducts || []).map((p) => p.id);
  const { data: creativePids } = allPids.length > 0
    ? await supabase.from('creatives').select('product_id').eq('store_id', storeId)
        .in('status', ['pending', 'approved', 'published']).in('product_id', allPids)
    : { data: [] };
  const hasAnyCreative = new Set((creativePids || []).map((c) => c.product_id));

  // Also extend hasBeach with all active product ids (batch query already ran above)
  // But we need to re-check with full product list — the earlier hasBeach only covered topProducts ids
  ...
  for (const p of allStoreProducts || []) {
    if (topProductIdSet.has(p.id)) continue; // already checked in topProducts loop
    if (hasBeachAll.has(p.id)) continue; // already has beach photo
    if (!hasAnyCreative.has(p.id)) continue; // no creatives yet — covered by product_no_creatives event

    const created = await createEventAndProposal(supabase, {
      storeId, productId: p.id, type: 'product_no_beach_photo', severity: 'medium',
      title: `${p.title} — missing beach photo`,
      description: `Product has creatives but no beach-style photo`,
      metadata: {},
      proposalType: 'generate_beach_photo',
      proposalTitle: `Generate beach photo for "${p.title}"`,
      proposalDescription: `Has creatives, 0 beach. Beach photos perform well for swimwear.`,
...
async function createEventAndProposal(supabase, opts) {
  ...
  // Cooldown: skip if same event was dismissed/rejected in the last 7 days
  const cooldownDate = new Date(Date.now() - 7 * 86400000).toISOString();
  const { data: dismissed } = await supabase.from('proposals').select('id')
    .eq('store_id', storeId).eq('product_id', productId).eq('type', proposalType)
    .eq('status', 'rejected').gte('updated_at', cooldownDate).limit(1).single();
