...
// Exposure lift applied to every finished Product Catalog image. The generated shots tend
// to land a touch dark (especially black garments); prompting "BRIGHT" hasn't moved the
// needle enough, so we lift it deterministically here. 1.18/13 was overexposing (sand/sky
// blown out) — backed off to a compromise: brighter than the original +10% but the
// highlights still hold detail. If it goes back to too dark, nudge up to ~1.15/10; if it
// blows out again, back down to ~1.10/6.
const BRIGHTNESS_MULT = 1.12;  // ~+12% overall exposure
const SHADOW_LIFT = 8;          // raise the black point by ~8/255
