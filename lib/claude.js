import Anthropic from '@anthropic-ai/sdk';
import { createClient } from '@supabase/supabase-js';

const client = new Anthropic({ apiKey: process.env.ANTHROPIC_API_KEY });
const supabase = createClient(process.env.SUPABASE_URL, process.env.SUPABASE_SERVICE_ROLE_KEY);

async function getStoreKnowledge(storeId, productName = null) {
  if (!storeId) return '';

  // Load store-level skills
  const { data: storeSkills } = await supabase.from('store_skills').select('skill_type, content')
    .eq('store_id', storeId).is('product_name', null)
    .in('skill_type', ['brand-voice', 'audience-personas']);

  // Load product-specific skill if product name provided
  let productSkill = null;
  if (productName) {
    const slug = productName.toLowerCase().replace(/\s+/g, '-');
    const { data } = await supabase.from('store_skills').select('skill_type, content')
      .eq('store_id', storeId).eq('skill_type', `product-${slug}`).limit(1);
    if (data?.length) productSkill = data[0];
  }

  const parts = [];
  if (storeSkills?.length) parts.push(...storeSkills.map((s) => `[${s.skill_type}]\n${s.content}`));
  if (productSkill) parts.push(`[${productSkill.skill_type}]\n${productSkill.content}`);

  if (parts.length) return '\n\nBRAND KNOWLEDGE:\n' + parts.join('\n\n');

  // Fallback to raw insights
  const { data } = await supabase.from('store_knowledge').select('insights, category')
    .eq('store_id', storeId).order('processed_at', { ascending: false }).limit(20);
  if (!data?.length) return '';
  return '\n\nSTORE KNOWLEDGE:\n' + data.map((k) => `[${k.category}] ${k.insights}`).join('\n');
}

const GENERIC_SYSTEM_PROMPT = `You are a professional e-commerce copywriter.

Description structure:
1. Opening hook — name the pain or desire (1-2 sentences, emotionally compelling)
2. Product introduction — what it is and why it's different (2-3 sentences)
3. Key features — bullet points with BENEFITS, not just specs
4. Material & care — brief, factual
5. Sizing & fit — helpful guidance

Tags: product category, key feature, target occasion, season if relevant, body type benefit.

Variant naming standards:
- Sizes MUST be: S, M, L, XL, XXL, 2XL, 3XL (standardize from: small/sm/s → S, xlarge/x-large → XL, etc.)
- Colors MUST be in English, capitalized: Black, Navy, Beige, White, Red, Pink, Blue, Green, Grey, Brown, Cream, Burgundy, Olive, Khaki, Camel
- Common renames: BLK/blk/schwarz/noir → Black, WHT/wht/weiss/blanc → White, NVY → Navy
- Option labels: "Size" and "Color" (not "Velikost", "Barva", "Farbe", "Taille")

You MUST return ONLY valid JSON. No markdown, no explanation, no code fences.`;

async function buildSystemPrompt(storeId, storeName) {
  if (!storeId) return GENERIC_SYSTEM_PROMPT;

  // Load brand-voice skill from store_skills
  const { data: brandSkill } = await supabase.from('store_skills').select('content')
    .eq('store_id', storeId).eq('skill_type', 'brand-voice').is('product_name', null).single();

  if (brandSkill?.content) {
    return brandSkill.content + '\n\nYou MUST return ONLY valid JSON. No markdown, no explanation, no code fences.';
  }

  // Fallback: generic prompt with store name
  return `You are a professional e-commerce copywriter for ${storeName}.\n\n` + GENERIC_SYSTEM_PROMPT;
}

function buildOptimizationPrompt(rawProduct, brandContext, storeName) {
  const variantsInfo = rawProduct.variants
    ? `\nVariants:\n${JSON.stringify(rawProduct.variants.slice(0, 20), null, 2)}`
    : '';

  return `Rewrite this imported product listing for ${storeName} store.

ORIGINAL PRODUCT DATA:
Title: ${rawProduct.title}
Description: ${rawProduct.description || '(no description)'}
Price: $${rawProduct.price || 'unknown'}
Product type: ${rawProduct.product_type || 'unknown'}
Tags: ${rawProduct.tags || 'none'}
Images: ${rawProduct.image_count || 0} images available
${variantsInfo}

${brandContext ? `ADDITIONAL BRAND CONTEXT:\n${brandContext}\n` : ''}
Return ONLY valid JSON:
{
  "title": "optimized title in [NAME] | [Benefit] format",
  "description": "full HTML description with h3 headings, paragraphs, ul/li bullet points, and emotional copy",
  "seo_title": "max 60 chars, keyword-rich",
  "seo_description": "max 155 chars, compelling meta description",
  "tags": ["tag1", "tag2", "tag3"],
  "product_type": "category",
  "vendor": "${storeName}"${rawProduct.variants ? `,
  "variants": [
    { "id": "original_variant_id", "option1": "standardized Size", "option2": "standardized Color" }
  ],
  "option_labels": { "option1": "Size", "option2": "Color" }` : ''}
}`;
}

export async function optimizeProduct(rawProduct, brandContext = '', storeId = null) {
  const knowledge = await getStoreKnowledge(storeId, rawProduct.title);
  const fullContext = brandContext + knowledge;

  // Get store name for prompt personalization
  let storeName = 'our store';
  if (storeId) {
    const { data: store } = await supabase.from('stores').select('name').eq('id', storeId).single();
    if (store?.name) storeName = store.name;
  }

  const systemPrompt = await buildSystemPrompt(storeId, storeName);

  const response = await client.messages.create({
    model: 'claude-sonnet-4-20250514',
    max_tokens: 3000,
    system: systemPrompt,
    messages: [{
      role: 'user',
      content: buildOptimizationPrompt(rawProduct, fullContext, storeName),
    }],
  });

  const text = response.content[0].text;
  let cleaned = text.trim();
  if (cleaned.startsWith('```')) {
    cleaned = cleaned.replace(/^```json?\n?/, '').replace(/\n?```$/, '');
  }

  return JSON.parse(cleaned);
}
