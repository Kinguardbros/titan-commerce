...
async function getStoreKnowledge(storeId) {
  if (!storeId) return '';
  // Prefer compiled skills, fall back to raw insights
  const { data: skills } = await supabase.from('store_skills').select('skill_type, content')
    .eq('store_id', storeId).in('skill_type', ['brand-voice', 'product-knowledge', 'audience-personas']);
  if (skills?.length) {
    return '\n\nBRAND KNOWLEDGE:\n' + skills.map((s) => `[${s.skill_type}]\n${s.content}`).join('\n\n');
  }
