...
Return ONLY the category name (one word): Brand, Audience, Products, Creative, Ads, or Logos`,
  ...
  const category = (response.content.find((b) => b.type === 'text')?.text ?? '').trim();
...
/**
 * Identify which product a document is about (for Products category)
 */
export async function identifyProduct(text, filename, anthropic) {
  const response = await anthropic.messages.create({
    model: 'claude-sonnet-5',
    max_tokens: 100,
    messages: [{
      role: 'user',
      content: `Based on this document content and filename, identify the specific PRODUCT NAME this is about.

Return ONLY the product name — a short, clean name like "Mathilda", "Elara", "Summer Dress", "High Waist Bikini".
If you can't identify a specific product, return "General".

Filename: ${filename}
Content (first 1500 chars): ${text.slice(0, 1500)}`,
    }],
  });

  const name = (response.content.find((b) => b.type === 'text')?.text ?? '').trim().replace(/[^a-zA-Z0-9\s-]/g, '').slice(0, 50);
  return name || 'General';
}
