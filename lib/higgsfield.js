...
/**
 * Generate an image using Flux Kontext Max (text-to-image, no reference image needed).
 * Better quality for product photos, lifestyle, ad creatives.
 */
export async function generateFluxKontext({ prompt, aspectRatio = '4:3', seed = null }) {
  const creds = process.env.HF_CREDENTIALS;
  const body = { prompt, aspect_ratio: aspectRatio, safety_tolerance: 6 };
  if (seed) body.seed = seed;

  const res = await fetch('https://platform.higgsfield.ai/flux-pro/kontext/max/text-to-image', {
    method: 'POST',
  ...
  if (!requestId) throw new Error('No request ID from Flux Kontext');

  const result = await pollUntilDone(requestId);
  return { url: result.url, jobId: requestId };
}

// Styles that use Flux Kontext Max instead of Soul
 ...
 */
export function buildPrompt({ product_name, price, headline, hook, age_desc = 'Woman age 35-55' }) {
...
Show: ${age_desc}, normal/curvy body type (not model-thin), wearing
 ...
 */
const STYLE_PROMPTS = {
  ad_creative: ({ product_name, price, custom_prompt, age_desc }) =>
    `Campaign-ready Meta ad creative for Elegance House.

Product: ${product_name}${price ? ` — ${price}` : ''}

Use the provided product photo as reference. Keep the product recognizable.
${age_desc}, confident, elegant. Warm gold (#d4a853) and cream (#f5f0e8) tones.
...
${custom_prompt ? `\nAdditional instructions: ${custom_prompt}` : ''}`.trim(),
  ...
  product_photo_beach: ({ product_name, price, custom_prompt, age_desc }) =>
    `Beach setting product photography.

Product: ${product_name}${price ? ` — ${price}` : ''}

${age_desc} wearing the product on a beach. Bright natural afternoon daylight.
LIGHTING: Sun high in the sky, bright clean daylight. Sun is BEHIND the camera, illuminating the model and product FROM THE FRONT. The product must be fully lit, bright, high-key, and clearly visible — NOT backlit, NOT silhouetted. No golden hour, no warm orange tones — clean neutral daylight that shows every fabric detail and true colors.
...
Warm tones: golden, sandy, turquoise. Natural hair.
IMPORTANT: The model's hands must be EMPTY. No objects in hands — no shells, no drinks, no sunglasses, no phone, no props. Arms relaxed at sides or on hips.
1080x1080, photorealistic, aspirational but achievable.
${custom_prompt ? `\nAdditional instructions: ${custom_prompt}` : ''}`.trim(),

  lifestyle: ({ product_name, price, custom_prompt, age_desc }) =>
    `Lifestyle fashion photography for Elegance House social media.

Product: ${product_name}${price ? ` — ${price}` : ''}

Use the provided product photo as reference. Keep the product recognizable.
${age_desc} wearing the product in a real-life setting:
...
${custom_prompt ? `\nAdditional instructions: ${custom_prompt}` : ''}`.trim(),
  ...
  review_ugc: ({ product_name, price, custom_prompt, age_desc }) =>
...
${age_desc}, looks like an everyday person — not a model.
...
${custom_prompt ? `\nAdditional instructions: ${custom_prompt}` : ''}`.trim(),
  ...
  static_clean: ({ product_name, price, custom_prompt, age_desc }) =>
...
${age_desc} wearing the product, standing straight, facing camera.
...
${custom_prompt ? `\nAdditional instructions: ${custom_prompt}` : ''}`.trim(),
  ...
  static_split: ({ product_name, price, custom_prompt, age_desc }) =>
    `Split-screen fashion ad — model on one side, product detail on the other.

Product: ${product_name}${price ? ` — ${price}` : ''}

Use the provided product photo as reference. Keep the product recognizable.
LEFT HALF: ${age_desc} wearing the product, full body, warm cream background.
...
${custom_prompt ? `\nAdditional instructions: ${custom_prompt}` : ''}`.trim(),
  ...
  static_urgency: ({ product_name, price, custom_prompt, age_desc }) =>
    `High-converting sales ad with urgency feel.

Product: ${product_name}${price ? ` — ${price}` : ''}

Use the provided product photo as reference. Keep the product recognizable.
${age_desc} wearing the product, confident pose, looking at camera.
...
async function loadSkillChain(storeId, primarySkillType) {
  if (!storeId || !primarySkillType) { console.log('[skill-chain] Skip: no storeId or skillType'); return ''; }

  console.log('[skill-chain] Loading:', primarySkillType, 'for store:', storeId);
  const { data: primary, error: primaryErr } = await supabaseHF.from('store_skills').select('content')
    .eq('store_id', storeId).eq('skill_type', primarySkillType).is('product_name', null).single();

  if (primaryErr) console.error('[skill-chain] Error loading primary skill:', primaryErr.message);
  if (!primary?.content) { console.log('[skill-chain] No content found for:', primarySkillType); return ''; }
  console.log('[skill-chain] Loaded:', primarySkillType, 'length:', primary.content.length);
...
export async function buildStyledPrompt({ product_name, price, style, custom_prompt = '', showModel = true, feedback = '', textOverlay = 'none', overlayText = '', storeId = null, audience = null }) {
  // Will be populated from audience-personas skill below; drives age_desc in style prompts
  let matchedPersonaAge = null;
  ...
  // Audience/persona targeting — load personas skill and inject matched persona details
  if (audience && storeId) {
    try {
      const { data: ap } = await supabaseHF.from('store_skills').select('content')
        .eq('store_id', storeId).eq('skill_type', 'audience-personas').is('product_name', null).single();
      if (ap?.content) {
        // Try Format 1: "Name (Age) — Label"
        const rx1 = /(?:###?\s*|(?:\*\*))?\s*(\w+)\s*\((\d+)\)\s*(?:\*\*)?\s*[—–-]\s*(.+?)(?:\n|$)/g;
        let matched = null; let m;
        while ((m = rx1.exec(ap.content)) !== null) {
          if (m[1].toLowerCase() === audience.toLowerCase()) { matched = { name: m[1], age: m[2], label: m[3].trim().replace(/\*+$/, '') }; break; }
        }
        // Try Format 2: "### Persona N: Name "Label"\n- **Age**: N"
        if (!matched) {
          const rx2 = /###\s*Persona\s*\d+:\s*(\w+)\s*"([^"]+)"\s*\n[^]*?(?:\*\*Age\*\*|Age):\s*(\d+)/g;
          while ((m = rx2.exec(ap.content)) !== null) {
            if (m[1].toLowerCase() === audience.toLowerCase()) { matched = { name: m[1], age: m[3], label: m[2].trim() }; break; }
          }
        }
        if (matched) {
          const age = parseInt(matched.age, 10);
          matchedPersonaAge = age;
          const features = age >= 55 ? 'visible grey or silver hair strands, deep crow\'s feet, nasolabial folds, mature skin texture, sagging/softer jawline — a clearly mature woman in her late 50s or 60s'
            : age >= 45 ? 'visible fine lines around eyes and mouth, some grey hair strands, mature skin with pores and natural texture, softer facial contours — a clearly mature woman in her late 40s or 50s'
            : age >= 35 ? 'subtle fine lines around eyes, mature adult features, natural skin texture, fully adult facial structure — a woman in her late 30s or early 40s'
            : 'natural young adult features';
          custom_prompt = (custom_prompt ? custom_prompt + '\n\n' : '') +
            `CRITICAL AGE OVERRIDE — this persona's age is the ONLY valid age for the model in this image.\n` +
            `IGNORE any other age ranges (like "age 35-55", "age 40-55", "age 35-50") that appear anywhere else in this prompt — those are template defaults that DO NOT APPLY when a persona is specified.\n\n` +
            `TARGET PERSONA — the model MUST look like this specific person:\n` +
            `- Name: ${matched.name}\n` +
            `- Age: ${matched.age} years old — the model MUST clearly look ${matched.age} years old\n` +
            `- Required visible features: ${features}\n` +
            `- Type: ${matched.label}\n` +
            `- Do NOT generate a younger model. If in doubt, go OLDER, not younger.\n\n` +
            `Full persona reference:\n${ap.content.slice(0, 2500)}`;
        } else {
          custom_prompt = (custom_prompt ? custom_prompt + '\n\n' : '') +
            `CRITICAL AGE OVERRIDE — ignore any other age ranges in this prompt; this persona's age is the only valid one.\n\n` +
            `TARGET PERSONA: "${audience}". Persona reference:\n${ap.content.slice(0, 2500)}`;
        }
      } else {
        custom_prompt = (custom_prompt ? custom_prompt + '\n\n' : '') +
          `MODEL MUST represent this specific persona: "${audience}". Match her age, body type, and emotional energy.`;
      }
    } catch (err) {
      console.error('[higgsfield] Failed to load audience-personas skill:', err.message);
      custom_prompt = (custom_prompt ? custom_prompt + '\n\n' : '') +
        `MODEL MUST represent this specific persona: "${audience}". Match her age, body type, and emotional energy.`;
    }
  } else if (audience) {
  ...
  // Build age descriptor — uses persona age if selected, else a wide default.
  // Default is intentionally broad so non-persona flows stay flexible.
  const ageDesc = matchedPersonaAge != null
    ? `Woman exactly ${matchedPersonaAge} years old`
    : 'Woman age 35-55';

  // Mathilda specialized prompt
  if (isMathilda && style === 'ad_creative' && showModel && textOverlay === 'none') {
    let prompt = buildPrompt({ product_name, price, headline: product_name, hook: custom_prompt || 'Elegance, delivered.', age_desc: ageDesc });
    if (feedback) prompt += `\n${feedback}`;
    return prompt;
  }
...
Show: ${ageDesc}, natural curvy body with soft midsection (NOT a fitness
...
Output: 1080x1080, photorealistic, no text overlay.`.trim();
  ...
  let prompt = builder({ product_name, price, custom_prompt, age_desc: ageDesc });
  ...
  if (textOverlay === 'none') {
    prompt += '\n\nCRITICAL: Do NOT add ANY text, words, letters, numbers, watermarks, logos, branding, brand names, labels, badges, medallions, gold/bronze emblems, crests, decorative metal elements, hang tags, price tags, or typography anywhere in the image — not on the product, not on the background, not floating, not as accessories. The product must have ZERO visible branding or decorative elements that are not part of the original design. Absolutely NO badges or emblems of any kind. The image must be completely clean.';
  } else if (textOverlay === 'custom' && overlayText) {
