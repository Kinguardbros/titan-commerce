  ...
  if (audience && storeId) {
    try {
      ...
      if (ap?.content) {
        ...
        if (matched) {
          custom_prompt = (custom_prompt ? custom_prompt + '\n\n' : '') +
            `TARGET PERSONA — the model in the image MUST look like this specific person:\n` +
            `- Name: ${matched.name}\n` +
            `- Age: ${matched.age} years old — the model MUST clearly look ${matched.age} years old, with visible age-appropriate features (skin texture, fine lines, mature facial structure for 35+, crow's feet and grey strands for 45+, etc.). Do NOT generate a younger model.\n` +
            `- Type: ${matched.label}\n\n` +
