...
export async function buildStyledPrompt({ product_name, price, style, custom_prompt = '', showModel = true, feedback = '', textOverlay = 'none', overlayText = '', storeId = null }) {
  // Load skills: per-function → store-level → product-specific → raw fallback
  if (storeId) {
    try {
      const parts = [];

      // 1. Per-function studio skill (most specific)
      const functionSkillType = STYLE_TO_SKILL[style];
      if (functionSkillType) {
        const { data: funcSkill } = await supabaseHF.from('store_skills').select('content')
          .eq('store_id', storeId).eq('skill_type', functionSkillType).is('product_name', null).limit(1).single();
        if (funcSkill?.content) parts.push(`[${functionSkillType}]\n${funcSkill.content}`);
      }

      // 2. Store-level skills (brand-voice always, creative-direction if no function skill)
      const storeTypes = ['brand-voice'];
      if (!parts.length) storeTypes.push('creative-direction');
      const { data: storeSkills } = await supabaseHF.from('store_skills').select('skill_type, content')
        .eq('store_id', storeId).is('product_name', null).in('skill_type', storeTypes);
      if (storeSkills?.length) parts.push(...storeSkills.map((s) => s.content));
