...
export function createShopifyClient(storeUrl, token) {
  ...
  async function getRevenueDelta(days = 7) {
    const current = await getRevenueSummary(days);
    const total2x = await getRevenueSummary(days * 2);
  ...
  async function getTopProductsWithCreatives(days = 7, limit = 10) {
    const orders = await fetchOrders(days);
    const prevOrders = await fetchOrders(days * 2);
    const curMap = {};
    ...
    for (const p of products) {
      const { data: dbP } = await supabase.from('products').select('id').ilike('title', `%${p.title.split('|')[0].trim()}%`).limit(1).single();
      if (dbP) {
        const { count: total } = await supabase.from('creatives').select('*', { count: 'exact', head: true }).eq('product_id', dbP.id);
        const { count: approved } = await supabase.from('creatives').select('*', { count: 'exact', head: true }).eq('product_id', dbP.id).eq('status', 'approved');
        p.creative_count = total || 0; p.approved_count = approved || 0; p.product_id = dbP.id;
