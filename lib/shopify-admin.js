...
export function createShopifyClient(storeUrl, token) {
  ...
  async function getTopProductsWithCreatives(days = 7, limit = 10) {
    ...
    // Helper: calculate refund total per order
    const orderRefundTotal = (o) => (o.refunds || []).reduce((sum, r) =>
      sum + (r.transactions || []).reduce((ts, tx) => ts + parseFloat(tx.amount || 0), 0), 0);

    const curMap = {};
    for (const o of orders) {
      const refund = orderRefundTotal(o);
      const lineCount = (o.line_items || []).length;
      const refundPerLine = lineCount > 0 ? refund / lineCount : 0;
      for (const li of o.line_items || []) {
        if (!curMap[li.title]) curMap[li.title] = { title: li.title, units: 0, revenue: 0 };
        curMap[li.title].units += li.quantity;
        curMap[li.title].revenue += parseFloat(li.price || 0) * li.quantity - refundPerLine;
      }
    }
    const prevMap = {};
    ...
    for (const o of prevOrders) {
      ...
      for (const li of o.line_items || []) {
        ...
        prevMap[li.title].revenue += parseFloat(li.price || 0) * li.quantity - refundPerLine;
      }
    }
  ...
  async function getRecentOrders(limit = 10) {
    ...
    return data.orders.map((o) => ({
      id: o.id, name: o.name, total: parseFloat(o.total_price || 0), currency: o.currency,
      status: o.fulfillment_status || 'unfulfilled', created_at: o.created_at,
      items: (o.line_items || []).slice(0, 3).map((li) => ({ title: li.title, quantity: li.quantity })),
      shipping: (o.shipping_lines || []).reduce((s, l) => s + parseFloat(l.price || 0), 0),
      payment_gateway: o.payment_gateway_names?.[0] || 'unknown',
      financial_status: o.financial_status,
      refund_amount: (o.refunds || []).reduce((s, r) => s + (r.transactions || []).reduce((ts, t) => ts + parseFloat(t.amount || 0), 0), 0),
    }));
  }
  ...
  async function createProduct(productData) {
    return rest('products.json', 'POST', {
      product: {
        ...
        variants: [{ price: productData.price || '0.00' }],
        images: (productData.images || []).map((src) => ({ src })),
      },
    });
  }

  async function getFullProduct(shopifyProductId) {
    const data = await rest(`products/${shopifyProductId}.json`);
    return data?.product || null;
  }

  async function getProductMetafields(shopifyProductId) {
    const data = await rest(`products/${shopifyProductId}/metafields.json`);
    return data?.metafields || [];
  }

  async function getMetafield(shopifyProductId, namespace, key) {
    const metafields = await getProductMetafields(shopifyProductId);
    return metafields.find((m) => m.namespace === namespace && m.key === key) || null;
  }

  async function updateMetafield(shopifyProductId, namespace, key, value, type = 'multi_line_text_field') {
    // Check if metafield already exists
    const existing = await getMetafield(shopifyProductId, namespace, key);
    if (existing) {
      return rest(`products/${shopifyProductId}/metafields/${existing.id}.json`, 'PUT', {
        metafield: { id: existing.id, value, type },
      });
    }
    return rest(`products/${shopifyProductId}/metafields.json`, 'POST', {
      metafield: { namespace, key, value, type },
    });
  }

  async function updateProductStatus(shopifyProductId, status) {
    return rest(`products/${shopifyProductId}.json`, 'PUT', { product: { status } });
  }

  async function updateProductImages(shopifyProductId, images) {
    return rest(`products/${shopifyProductId}.json`, 'PUT', { product: { images } });
  }
  ...
  return {
    ...
    updateProduct, updateVariant, updateProductOptions, getProductVariants, bulkUpdateVariantPrices, createProduct,
    getFullProduct, getProductMetafields, getMetafield, updateMetafield, updateProductStatus, updateProductImages,
...
export const createProduct = (...a) => defaultClient.createProduct(...a);
