...
function prioritizeReviews(reviews) {
  ...
  return [...reviews].sort((a, b) => {
    ...
    // then rating DESC (worst drops off the tail when cap applied)
...
const ALLOWED_PHOTO_HOSTS = /^(?:[\w-]+\.)?media-amazon\.com$|^(?:[\w-]+\.)?ssl-images-amazon\.com$|^(?:[\w-]+\.)?images-amazon\.com$|^rewimg-(?:eu|us)\.kwcdn\.com$/;
...
const ALLOWED_SOURCES = new Set(['amazon', 'temu']);
...
export async function import_amazon_reviews(req, res) {
  ...
  // Download + validate + upload photos in parallel — sequential loop was O(N * ~2s) and hit
  // Vercel's 60s timeout at MAX_REVIEWS=50. Parallel keeps total time bounded by the slowest
  // single photo. Preserves the SSRF allow-list gate (I-3), per-photo error isolation, and
  // 8 MB size cap.
