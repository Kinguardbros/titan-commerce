...
import { supabase, validateImageBuffer, uploadReviewImage, dropExistingDuplicates, stripLoneSurrogates } from './reviews-shared.js';
...
const ALLOWED_PHOTO_HOSTS = /^(?:[\w-]+\.)?media-amazon\.com$|^(?:[\w-]+\.)?ssl-images-amazon\.com$|^(?:[\w-]+\.)?images-amazon\.com$|^rewimg-(?:eu|us)\.kwcdn\.com$|^cdn-review\.cupshe\.com$|^media\.cupshe\.com$|^judgeme\.imgix\.net$|^(?:[\w-]+\.)?judge\.me$/;
...
const ALLOWED_SOURCES = new Set(['amazon', 'temu', 'cupshe', 'judgeme']);
...
export async function import_amazon_reviews(req, res) {
  ...
  const candidates = prioritizedInput.map((rv) => {
    ...
    return {
      store_id, product_id,
      // stripLoneSurrogates AFTER the slice — the slice is what can split an emoji in half,
      // and one broken row would fail the whole batch insert (see helper for detail).
      // Applied here rather than client-side so older userscript versions are covered too.
      author: stripLoneSurrogates(anonymizeAuthor(rv.author)),
      rating: rv.rating,
      title: stripLoneSurrogates((rv.title || '').slice(0, 200)),
      body: stripLoneSurrogates((rv.body || '').slice(0, 2000)),
