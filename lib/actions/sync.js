...
// Shopify REST pages via the Link header, not `since_id`. The products list is not
// guaranteed to be ordered by id, so a `since_id` loop both repeats and skips rows
// (measured on Clara Atelier 2026-09-14: 438 rows fetched, 281 unique, 285 real).
// `page_info` cursors are the documented way and cannot drift.
async function fetchAllAdminPages(firstUrl, token, key) {
  let url = firstUrl;
  const out = [];
  while (url) {
    const resp = await fetch(url, { headers: { 'X-Shopify-Access-Token': token } });
...
export async function sync_products(req, res) {
  ...
  let allProducts = [];
  if (adminToken) {
    allProducts = await fetchAllAdminPages(
      `https://${storeUrl}/admin/api/2024-01/products.json?limit=250`, adminToken, 'products'
    );
  } else {
    let page = 1;
    let hasMore = true;
  ...
  for (const p of allProducts) {
    try {
      ...
      // P0-4 (Docs/AUDIT-2026-08.md): this used to be console.error only — invisible
      // to Dan. Surface it in pipeline_log so failed syncs (e.g. a handle collision
      // with another store) show up in the Cockpit terminal log instead of silently
      // dropping the product. Own try/catch so a pipeline_log write failure can't
      // abort the rest of the sync loop.
      try {
        await supabase.from('pipeline_log').insert({
          agent: 'SCRAPER', level: 'warn', store_id: storeId,
          message: `Product sync failed for "${p.handle}": ${err.message}`,
          metadata: { handle: p.handle, shopify_id: p.id },
          user_id: req.user?.user_id || null, initiator: 'user',
        });
      } catch (logErr) {
        console.error(`[sync] Failed to write pipeline_log for ${p.handle}:`, logErr.message);
      }
    }
  }
