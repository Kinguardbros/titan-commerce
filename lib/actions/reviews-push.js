...
export async function push_reviews_to_shopify(req, res) {
  ...
  // We keep the SUMMARY (count, avg) faithful to the full list (all pushed reviews contribute
  // to the storefront's ★ average), but the JSON array itself is limited to top-N by
  // (has photo DESC, rating DESC, review_date DESC). Storefront renders best reviews first;
  // reviews outside top-N stay in Titan DB but aren't shown on storefront.
  const SHOPIFY_METAFIELD_TOP_N = 200;
  const topN = [...list].sort((a, b) => {
  ...
  const r2 = await client.updateMetafield(product.shopify_id, 'custom', 'reviews_summary', JSON.stringify({ count: list.length, average: avg, shown: topN.length }), 'json');
  if (!r1 || !r2) {
  ...
  await supabase.from('pipeline_log').insert({
    ...
    message: `Pushed ${list.length} review(s) (avg ${avg}★) for "${product.title}"${trimmedNote}`,
  });

  return res.status(200).json({ ok: true, count: list.length, average: avg, shown: topN.length });
}
