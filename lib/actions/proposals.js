...
export async function proposals_list(req, res) {
  ...
  // Non-admins must scope to a store they have access to — an unscoped call would
  // otherwise leak proposals across every store to a non-admin caller.
...
export async function approve_proposal(req, res) {
  ...
  await supabase.from('pipeline_log').insert({ agent: 'AGENT', level: 'info', store_id: proposal.store_id, message: `Executed: ${proposal.title}`, user_id: req.user?.user_id || null, initiator: 'user' });
...
export async function reject_proposal(req, res) {
  ...
  await supabase.from('pipeline_log').insert({ agent: 'AGENT', level: 'warn', store_id: proposal?.store_id, message: `Dismissed: ${proposal?.title}${reason ? ' — ' + reason : ''}`, user_id: req.user?.user_id || null, initiator: 'user' });
...
export async function approve_all_proposals(req, res) {
  ...
  for (const pid of proposal_ids) {
    try {
      ...
      await supabase.from('pipeline_log').insert({ agent: 'AGENT', level: 'info', store_id: p.store_id, message: `Bulk executed: ${p.title}`, user_id: req.user?.user_id || null, initiator: 'user' });
      executed++;
...
export async function scan_events(req, res) {
  ...
  await supabase.from('pipeline_log').insert({ agent: 'AGENT', level: 'info', store_id, message: `Scan complete: ${eventsCreated} events, ${proposalsCreated} proposals created`, user_id: req.user?.user_id || null, initiator: 'user' });
