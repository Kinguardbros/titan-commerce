...
export async function store_docs(req, res) {
  ...
  // Only store_name (display name) is available here, not store_id — resolve it to
  // check store access, same pattern as the update_cogs product_id → store_id lookup.
  const { data: storeRow } = await supabase.from('stores').select('id').eq('name', storeName).single();
  if (!storeRow) return res.status(404).json({ error: 'Store not found' });
  if (!hasStoreAccess(req.user, storeRow.id)) {
