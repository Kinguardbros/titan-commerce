  ...
  for (const c of pending) {
    ...
    try {
      ...
      if (result.status === 'completed' && result.url) {
        ...
        // Catalog images (crop to the chosen framing + gentle brightness lift), and promote
        // to a permanent Storage URL. Fire-and-forget — don't block the poll response.
        (async () => {
          try {
            ...
            if (c.style === 'product_catalog') {
              const processed = await processCatalogImage(buf, meta.framing_crop || null);
              if (processed !== buf) { buf = processed; contentType = 'image/jpeg'; }
            }
            const { error: uploadErr } = await supabase.storage.from('creatives').upload(path, buf, { contentType, upsert: true });
