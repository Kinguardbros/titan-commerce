...
import { checkFalJob } from '../fal.js';
...
export async function poll_generations(req, res) {
  ...
  let query = supabase.from('creatives')
    .select('id, hf_job_id, metadata, storage_path')
  ...
  for (const c of pending) {
    ...
    // Hard timeout: if a job has been "generating" longer than MAX_JOB_AGE_MS,
    // give up and mark failed so the UI stops showing a spinner forever.
    const submittedAt = meta.submitted_at ? new Date(meta.submitted_at).getTime() : null;
    if (submittedAt && (Date.now() - submittedAt) > MAX_JOB_AGE_MS) {
      await supabase.from('creatives').update({ status: 'failed', metadata: { ...meta, error: `timeout after ${Math.round((Date.now() - submittedAt) / 1000)}s` } }).eq('id', c.id);
      failed++;
      console.warn('[poll_generations] hard timeout for', c.id, 'age:', Math.round((Date.now() - submittedAt) / 1000) + 's');
      continue;
    }
