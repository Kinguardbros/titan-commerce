  ...
  for (const c of pending) {
    ...
    try {
      ...
      if (result.status === 'completed' && result.url) {
        ...
        // For product_catalog (v1): post-processing changes the image dimensions (9:16 fal output
        // is cropped to 4:5). Showing the raw fal.ai URL first would briefly display the wrong
        // aspect ratio until the background crop+upload finishes, so we do it SYNCHRONOUSLY here
        // and only flip status once the cropped Storage URL is ready. Adds ~1-2s to this poll
        // iteration but the user sees the correct 4:5 image immediately.
        //
        // Other catalog flows (v2-v8) and non-catalog flows have no post-processing, so they
        // keep the optimistic flow: flip status now with the fal.ai URL, upload to Storage in
        // the background.
        ...
        if (needsSyncCrop) {
          try {
            ...
            const processed = await processCatalogImage(buf, meta.framing_crop || null, meta.garment_length || null);
          ...
          // Optimistic: show fal.ai URL immediately, upload to Storage in background.
          await supabase.from('creatives').update({ status: 'pending', file_url: result.url, storage_path: path }).eq('id', c.id);
          completed++;
          (async () => {
            try {
