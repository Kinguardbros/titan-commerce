...
import { cropImageBuffer } from '../avatar-crop.js';
  ...
  for (const c of pending) {
    ...
    try {
      ...
      if (result.status === 'completed' && result.url) {
        ...
        // Background: download from fal.ai (temporary URL, ~1h TTL), optionally crop to the
        // chosen Product Catalog framing, and promote to a permanent Storage URL.
        ...
        (async () => {
          try {
            ...
            if (meta.framing_crop) {
              const cropped = await cropImageBuffer(buf, meta.framing_crop);
              if (cropped !== buf) { buf = cropped; contentType = 'image/jpeg'; }
            }
