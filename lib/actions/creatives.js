    ...
    try {
      ...
      if (result.status === 'completed' && result.url) {
        try {
          ...
          await supabase.storage.from('creatives').upload(path, buf, { contentType: 'image/png', upsert: true });
          const { data: pub } = supabase.storage.from('creatives').getPublicUrl(path);
          await supabase.from('creatives').update({ status: 'pending', file_url: pub.publicUrl, storage_path: path }).eq('id', c.id);
          completed++;
        } catch (upErr) {
          console.error('[poll_generations] upload failed for', c.id, upErr.message);
          await supabase.from('creatives').update({ status: 'failed', metadata: { ...meta, error: `upload: ${upErr.message}` } }).eq('id', c.id);
          failed++;
        }
