...
export async function generate_avatar(req, res) {
  ...
  const prompt = fullDescription.startsWith('Professional') || fullDescription.startsWith('Full body')
    ? fullDescription
    : `Full body photograph of a real woman, age ${age}. Body: ${bodyDetails}. Head to knees visible, standing naturally with weight on one hip. Neutral beige studio background, warm soft lighting. Wearing simple black fitted activewear. Natural relaxed expression, warm smile, looking at camera. Real person, natural skin texture, age-appropriate face with expression lines. Shot on 85mm portrait lens, soft bokeh background.`;
