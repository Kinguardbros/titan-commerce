...
export async function generate_avatar(req, res) {
  ...
  const prompt = fullDescription.startsWith('Professional') || fullDescription.startsWith('Full body')
    ? fullDescription
    : `Full body photograph of a real woman, age ${age}. ${bodyDetails}. Head to knees visible. She is NOT a fashion model — she looks like a real customer, a real person you would meet at the beach. Standing naturally with weight on one hip, arms relaxed. Wearing simple black fitted activewear. Warm inviting studio lighting, clean neutral background. Sun-kissed natural skin, age-appropriate face with expression lines. No retouching, no airbrushing — real skin, real body. Relaxed confident expression, warm smile. Shot on 85mm portrait lens, soft bokeh. Photorealistic, authentic.`;
  ...
  const model = hasReference ? 'fal-ai/nano-banana-2/edit' : 'fal-ai/flux/schnell';
  const imageUrl = hasReference ? existingAvatar.reference_url : undefined;

  // Generate 4 angles: front, back, left profile, right profile
  const angles = [
    'Front view, facing camera directly.',
    'Back view, facing away from camera, looking over right shoulder.',
    'Left side profile, facing camera-left.',
    'Right side profile, facing camera-right.',
  ];
  const variantPromises = angles.map(angle =>
    generateFal({ model, prompt: `${prompt} ${angle}`, imageUrl, numImages: 1, aspectRatio: '4:5' })
  );
