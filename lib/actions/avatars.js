...
export async function generate_avatar(req, res) {
  ...
  // If description is already a full prompt (from AvatarBuilder), use as-is; otherwise wrap in template
  const prompt = description.startsWith('Professional headshot')
    ? description
    : `Professional headshot portrait photograph. Shoulders and face visible, neutral studio background with soft even lighting, natural relaxed expression, looking directly at camera.\n\nModel: ${description}\n\nStyle: Clean beauty portrait. Soft natural skin texture, minimal retouching. Shot on 85mm lens, f/2.8, shallow depth of field on background. The focus is on creating a RECOGNIZABLE, CONSISTENT face that can be used as reference across multiple product photo shoots.`;

  // Use text-to-image model for avatar generation (no reference image needed)
