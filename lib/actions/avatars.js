...
export async function generate_avatar(req, res) {
  ...
  // 2. Remaining 3 angles PARALLEL — use Nano Banana with front as reference
  ...
  const remainingResults = await Promise.all(
    remainingAngles.map(angle =>
      generateFal({
        ...
        prompt: `${prompt} ${angle} SAME WOMAN as the reference image — match her face, hair, skin tone, body type exactly.`,
        imageUrl: frontUrl,
        numImages: 1,
        aspectRatio: '4:5',
      })
