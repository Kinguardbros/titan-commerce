...
IDENTITY LOCK: The generated woman MUST look identical to the reference — same person, same face, same features, SAME BODY.`
  ...
  const prompt_3q = 'Show the EXACT SAME woman from the reference image, now from a 3/4 angle view (slightly turned to the left). Same hair, same skin, same face, and the SAME EXACT BODY proportions — do NOT slim her down, do NOT thin her waist, do NOT narrow her hips, preserve her actual weight and body shape exactly as in the reference. Wearing the SAME fitted neutral-tone activewear (tank top + bike shorts in a solid neutral color). FULL BODY shot from head to bare feet — entire body visible with feet at the bottom of the frame. Do NOT crop at the knees.';
  ...
  for (const row of pending) {
    ...
    try {
      ...
      if (row.status === 'generating') {
        ...
        // Submit 3/4 angle using front as reference (Banana 2 is fast enough — fire-and-forget anyway for consistency)
        let angleJob;
        try {
          angleJob = await submitFalJob({
            model: 'fal-ai/nano-banana-2/edit',
            prompt: meta.prompt_3q || 'Show the EXACT SAME woman from the reference image, now from a 3/4 angle view (slightly turned to the left). Same hair, same skin, same face, and the SAME EXACT BODY proportions.',
            imageUrl: frontUrl,
            numImages: 1,
            aspectRatio: '9:16',
          });
        } catch (err) {
          // Front succeeded but angle submit failed — keep front, mark angle skipped
          console.warn('[poll_avatar_generations] angle submit failed for', row.persona_name, '— keeping front only:', err.message);
          const allVariants = [...(row.variants || []), frontUrl];
          await supabase.from('persona_avatars').update({
            status: null,
            variants: allVariants,
            gen_metadata: { ...meta, angle_error: err.message, completed_at: new Date().toISOString() },
          }).eq('id', row.id);
          completed++;
          continue;
        }

        // Update row: variants += [frontUrl], status='angles', angle request tracked
        const allVariants = [...(row.variants || []), frontUrl];
        await supabase.from('persona_avatars').update({
          status: 'angles',
          variants: allVariants,
          angle_request_id: angleJob.requestId,
          angle_poll_base: angleJob.pollBase,
          gen_metadata: { ...meta, front_completed_at: new Date().toISOString(), front_url: frontUrl },
        }).eq('id', row.id);
        console.log('[poll_avatar_generations] front done for', row.persona_name, '— submitted 3/4 angle');
        // Don't count as completed yet — wait for angle. But next poll cycle will check angle status.
      }

      // STAGE 2: 3/4 angle in flight
      else if (row.status === 'angles') {
        if (!row.angle_poll_base || !row.angle_request_id) {
          // Shouldn't happen but handle gracefully — mark complete (front already saved)
          await supabase.from('persona_avatars').update({ status: null, gen_metadata: { ...meta, angle_error: 'missing angle poll info', completed_at: new Date().toISOString() } }).eq('id', row.id);
          completed++;
          continue;
        }

        const result = await checkFalJob(row.angle_poll_base, row.angle_request_id);
        ...
        if (result.status === 'failed') {
          // Angle failed but front already saved — mark complete with just the front variant
          await supabase.from('persona_avatars').update({
            status: null,
            gen_metadata: { ...meta, angle_error: result.error, completed_at: new Date().toISOString() },
          }).eq('id', row.id);
          completed++;
          console.warn('[poll_avatar_generations] angle failed for', row.persona_name, '— kept front only:', result.error);
          continue;
        }

        // result.status === 'completed' → download + upload angle
        const resp = await fetch(result.url);
        if (!resp.ok) throw new Error(`fal download ${resp.status}`);
        const buf = Buffer.from(await resp.arrayBuffer());
        if (buf.length < 1000) throw new Error(`buffer too small (${buf.length} bytes)`);

        const anglePath = `${storeName}/Avatars/${row.persona_name}/angle_${Date.now()}.jpg`;
        const { error: uploadErr } = await supabase.storage.from('store-docs').upload(anglePath, buf, { contentType: 'image/jpeg', upsert: true });
        if (uploadErr) throw new Error(`storage upload: ${uploadErr.message}`);
        const { data: urlData } = supabase.storage.from('store-docs').getPublicUrl(anglePath);
        const angleUrl = urlData?.publicUrl;
        if (!angleUrl) throw new Error('no public URL from Storage');

        // Append angle to variants, mark complete
        const allVariants = [...(row.variants || []), angleUrl];
        await supabase.from('persona_avatars').update({
          status: null,
          variants: allVariants,
          gen_metadata: { ...meta, completed_at: new Date().toISOString() },
        }).eq('id', row.id);
        completed++;
        console.log('[poll_avatar_generations] angle done for', row.persona_name, '— total variants:', allVariants.length);

        await supabase.from('pipeline_log').insert({
          agent: 'AVATAR', level: 'info', store_id,
          message: `Avatar generation completed for ${row.persona_name} (${allVariants.length} variants)`,
        });
      }
    } catch (err) {
