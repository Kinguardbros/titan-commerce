...
export async function generate_avatar(req, res) {
  ...
  // Extract only visual details from persona (age, body type) — skip emotional/quote content
  const ageMatch = fullDescription.match(/Age[:\s]*(\d+)/i);
  const age = ageMatch ? ageMatch[1] : '40';
  const bodyHints = fullDescription.match(/(?:body|size|shape|build|weight)[:\s]*([^\n.]{5,60})/i);
  const bodyDesc = bodyHints ? bodyHints[1].replace(/\*+/g, '').trim() : 'average natural body type';

  const prompt = fullDescription.startsWith('Professional') || fullDescription.startsWith('Full body')
    ? fullDescription
    : `Full body photograph of a woman, age ${age}. ${bodyDesc}. Head to knees visible, standing naturally with weight on one hip. Neutral beige studio background, warm soft lighting. Wearing simple black fitted activewear (sports bra and leggings). Natural relaxed expression, looking at camera. Real person, natural skin texture, age-appropriate face. 85mm portrait lens, soft bokeh background. Professional photography.`;

  // Nano Banana 2 requires image_urls — use placeholder when no reference exists
  const PLACEHOLDER = 'https://ercrkgfihqgrbkkqnoqy.supabase.co/storage/v1/object/public/store-docs/_system/placeholder.png';
  const variantPromises = Array.from({ length: 4 }, () =>
    generateFal({ model: 'fal-ai/nano-banana-2/edit', prompt, imageUrl: PLACEHOLDER, numImages: 1, aspectRatio: '4:5' })
  );
