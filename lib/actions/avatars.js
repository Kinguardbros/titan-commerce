...
export async function generate_avatar(req, res) {
  ...
  // Clean markdown from persona description and extract key details
  const clean = fullDescription
    .replace(/#{1,4}\s*/g, '')
    .replace(/\*{1,2}/g, '')
    .replace(/^-\s*/gm, '')
    .replace(/\n{2,}/g, '. ')
  ...
  const prompt = fullDescription.startsWith('Professional') || fullDescription.startsWith('Full body')
    ? fullDescription
    : `Full body photo, real woman, head to knees. Neutral studio, warm soft lighting. Natural relaxed pose, weight on one hip. Wearing plain nude underwear. ${clean}. Real person, age-appropriate face, natural body shape, natural skin. 85mm lens, soft bokeh.`;
