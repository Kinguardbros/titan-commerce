...
export async function generate_avatar(req, res) {
  ...
  const prompt = fullDescription.startsWith('Professional') || fullDescription.startsWith('Full body')
    ? fullDescription
    : `Full body reference photograph of a ${age}-year-old woman standing in front of a plain wall at home. Natural amateur photo, not a professional shoot. ${appearance} ${bodyDetails}.${weightDesc} FULL BODY shot — head to bare feet fully visible, feet at the bottom of the frame. Do NOT crop at the knees or waist. She is a regular ${age}-year-old woman, neither a model nor particularly photogenic. CLOTHING IS CRITICAL: She is wearing ONLY a plain BEIGE/NUDE skin-toned bra and BEIGE/NUDE skin-toned underwear briefs. The underwear color MUST match her skin tone — NOT black, NOT white, NOT grey, NOT any dark color. Simple plain nude-colored underwear, no patterns, no lace, no logos. Bare feet, no shoes. Arms relaxed at sides. Neutral indoor lighting, slightly uneven, like a normal hallway or bedroom. Plain beige or off-white wall background, no props, no furniture. ${faceDescriptor} No makeup or minimal natural makeup, no styled hair, no posed smile. Subtly neutral expression — she is not posing, just standing for a reference photo. Photograph shows imperfect real skin texture with ${bodyImperfections}. Snapshot quality, plain and unremarkable. Do NOT make her look like a model, influencer, swimsuit catalog, or professional photo. FINAL CHECK: The woman in this photograph is exactly ${age} years old — not older, not younger. Her underwear is BEIGE/NUDE colored, NOT black.`;
  ...
  const finalPrompt = hasReference
    ? `Recreate the EXACT same woman from the reference image. Keep her EXACT face, hair color, hair style, skin tone, body shape, and age — she must be immediately recognizable as the same person. FULL BODY shot from head to bare feet, standing in front of a plain beige or off-white wall. Arms relaxed at sides, neutral expression, facing camera directly. CLOTHING: plain BEIGE/NUDE skin-toned bra and BEIGE/NUDE briefs — NOT black, NOT white. Bare feet, no shoes. Natural indoor lighting. Snapshot quality, not a professional photo. IDENTITY LOCK: The generated woman MUST look identical to the reference — same person, same face, same features.`
    : prompt;

  // 1. Front view FIRST (anchor) — Nano Banana 2 for more natural output
  ...
  const remainingAngles = [
    'Show the EXACT SAME woman from the reference image, now from a 3/4 angle view (slightly turned to the left). Same hair, same skin, same body, same face. Wearing the same BEIGE/NUDE skin-toned underwear — NOT black. FULL BODY shot from head to bare feet — entire body visible with feet at the bottom of the frame. Do NOT crop at the knees.',
  ];
