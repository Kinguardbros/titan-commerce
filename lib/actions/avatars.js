...
export async function generate_avatar(req, res) {
  ...
  // Extract age only — body description from persona name lookup (safe, no quotes/emotions)
  ...
  const prompt = fullDescription.startsWith('Professional') || fullDescription.startsWith('Full body')
    ? fullDescription
    : `Full body photograph of a woman, age ${age}, natural curvy body type, average build. Head to knees visible, standing naturally with weight on one hip. Neutral beige studio background, warm soft lighting. Wearing simple black fitted activewear. Natural relaxed expression, looking at camera with warm smile. Real person, natural skin texture, age-appropriate face. Shot on 85mm portrait lens, soft bokeh background.`;
