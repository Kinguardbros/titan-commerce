...
export async function pending_optimizations(req, res) {
  ...
  if (req.query.store_id && !hasStoreAccess(req.user, req.query.store_id)) {
...
export async function optimize_product(req, res) {
  ...
  if (!hasStoreAccess(req.user, product.store_id)) {
    return res.status(403).json({ error: 'forbidden', hint: 'no access to this store' });
  }

  // Per-store + per-user rate limit (P0-8 from AUDIT-2026-08)
  if (!await rateLimit(`optimize_product:${product.store_id}:${req.user.user_id || req.user.username || 'master'}`, 30, 3600000)) {
...
export async function approve_optimization(req, res) {
  ...
  const { data: opt, error: oErr } = await supabase.from('product_optimizations')
    .select('*, product:products(shopify_id, title, store_id)').eq('id', optimization_id).single();
  if (oErr || !opt) return res.status(404).json({ error: 'Optimization not found' });
  if (!hasStoreAccess(req.user, opt.product?.store_id)) {
    return res.status(403).json({ error: 'forbidden', hint: 'no access to this store' });
  }
  ...
  await supabase.from('pipeline_log').insert({ agent: 'OPTIMIZER', level: 'info', message: `Approving & applying optimization for ${opt.product.title}`, user_id: req.user?.user_id || null, initiator: 'user' });
  ...
  await supabase.from('pipeline_log').insert({ agent: 'OPTIMIZER', level: 'info', message: `Approved & applied optimization for ${finalData.title || opt.product.title}`, user_id: req.user?.user_id || null, initiator: 'user' });
...
export async function reject_optimization(req, res) {
  ...
  const { data: opt } = await supabase.from('product_optimizations').select('product:products(title, store_id)').eq('id', optimization_id).single();
  if (!hasStoreAccess(req.user, opt?.product?.store_id)) {
    return res.status(403).json({ error: 'forbidden', hint: 'no access to this store' });
  }
...
export async function save_optimization(req, res) {
  ...
  const { data: opt } = await supabase.from('product_optimizations')
    .select('product:products(store_id)').eq('id', optimization_id).single();
  if (!hasStoreAccess(req.user, opt?.product?.store_id)) {
    return res.status(403).json({ error: 'forbidden', hint: 'no access to this store' });
  }
