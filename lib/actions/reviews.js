import { supabase, computeSummary, safePhotoUrl, deleteReviewPhoto, deleteReviewPhotosSettled, flagProductNeedsRepush } from './reviews-shared.js';
...
export async function add_review_manual(req, res) {
  const { store_id, product_id, author, rating, title, body, review_date, photo_url, photo_urls, verified } = req.body;
  ...
  // F08: photo_urls array (sanitized) or fall back to single photo_url mirrored into array
  const safeArr = Array.isArray(photo_urls)
    ? photo_urls.map(safePhotoUrl).filter(Boolean).slice(0, 10)
    : (photo_url ? [safePhotoUrl(photo_url)].filter(Boolean) : []);
  ...
  const { data, error } = await supabase.from('product_reviews').insert({
    ...
    photo_url: safeArr[0] || null,
    photo_urls: safeArr.length ? safeArr : null,
    verified: !!verified,
    source: 'manual',
    status: 'pending',
  }).select().single();
...
export async function update_review(req, res) {
  const { id, store_id, author, rating, title, body, review_date, photo_url, photo_urls, verified, helpful_count } = req.body;
  ...
  // F08: photo_urls JSONB array; each URL goes through safePhotoUrl (http(s) only,
  // blocks javascript:/data: XSS). Empty array → null so the "no photo" case is
  // distinguishable from "explicitly empty array" downstream.
  if (photo_urls !== undefined) {
    if (Array.isArray(photo_urls)) {
      const safe = photo_urls.map(safePhotoUrl).filter(Boolean).slice(0, 10);
      updates.photo_urls = safe.length ? safe : null;
    } else {
      updates.photo_urls = null;
    }
  }
...
export async function delete_review(req, res) {
  ...
  const { data: existing } = await supabase.from('product_reviews')
    .select('product_id, status, photo_url, photo_urls').eq('id', id).eq('store_id', store_id).single();
  ...
  // Remove all attached photos from Storage so abusive images don't linger on public URLs.
  // F08 photo_urls (JSONB array) is the source of truth; legacy photo_url may still be set
  // on rows created before F08 or as photo_urls[0] mirror — dedup before deleting.
  const toDelete = new Set([
    ...(Array.isArray(existing.photo_urls) ? existing.photo_urls : []),
    ...(existing.photo_url ? [existing.photo_url] : []),
  ]);
  await Promise.all(Array.from(toDelete).map((url) => deleteReviewPhoto(url)));
...
export async function set_review_status(req, res) {
  ...
  // Rejecting a review means its photos (if any) are never coming back — clean them out
  // of Storage right away instead of waiting for the 30-day orphan sweep. Read photo_url/
  // photo_urls BEFORE the update below (which nulls them on the row).
  const updates = { status, updated_at: new Date().toISOString() };
  let photoCleanup = null;
  if (status === 'rejected') {
    const { data: withPhotos } = await supabase.from('product_reviews')
      .select('photo_url, photo_urls').in('id', ids).eq('store_id', store_id);
    const urls = (withPhotos || []).flatMap((r) => [
      ...(Array.isArray(r.photo_urls) ? r.photo_urls : []),
      ...(r.photo_url ? [r.photo_url] : []),
    ]);
    if (urls.length) {
      // Promise.allSettled inside — one bad/missing Storage object doesn't abort the
      // rest, and never throws, so a Storage outage doesn't block the status transition.
      photoCleanup = await deleteReviewPhotosSettled(urls);
      if (photoCleanup.failed) {
        console.error('[reviews] set_review_status: photo cleanup failures', photoCleanup.failures);
      }
    }
  ...
  const photoSuffix = photoCleanup
    ? ` (${photoCleanup.removed} photo(s) removed from Storage${photoCleanup.failed ? `, ${photoCleanup.failed} failed` : ''})`
    : '';
  await supabase.from('pipeline_log').insert({
    store_id, agent: 'REVIEWS', level: 'info',
    message: `Set ${data?.length || 0} review(s) → ${status}${liveProducts.length ? ` (${liveProducts.length} product(s) need re-push)` : ''}${photoSuffix}`,
    user_id: req.user?.user_id || null, initiator: 'user',
  });

  // Per-file failures get their own warn-level entry (P1-16 initiator attribution) so
  // they're visible in the pipeline log without cluttering the main info-level line above.
  if (photoCleanup?.failed) {
    await supabase.from('pipeline_log').insert({
      store_id, agent: 'REVIEWS', level: 'warn',
      message: `Reject photo cleanup: ${photoCleanup.failed} of ${photoCleanup.removed + photoCleanup.failed} file(s) failed to remove from Storage`,
      user_id: req.user?.user_id || null, initiator: 'user',
    });
  }
