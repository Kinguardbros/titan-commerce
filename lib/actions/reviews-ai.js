...
import { supabase, dropExistingDuplicates, insertReviewsTolerant } from './reviews-shared.js';
...
export async function generate_reviews_ai(req, res) {
  ...
  if (rows.length) {
    ...
    // Pre-check only, not a lock — a concurrent generate/import/web-submit can still
    // insert the same review between this SELECT and the inserts below (TOCTOU).
    // insertReviewsTolerant inserts one row at a time and treats a 23505 (unique_
    // violation) as a late-caught duplicate instead of failing the whole batch
    // (previously: one collision 500'd the entire bulk .insert()).
    const fresh = await dropExistingDuplicates(supabase, store_id, product_id, rows);
    const result = await insertReviewsTolerant(supabase, fresh);
    inserted = result.inserted;
    skipped_duplicates = result.skipped_duplicates;
  }

  const dupNote = skipped_duplicates ? ` (${skipped_duplicates} duplicate skipped)` : '';
  await supabase.from('pipeline_log').insert({
    store_id, agent: 'REVIEWS', level: 'info',
    message: `AI-generated ${inserted} review(s) (${tone === 'mix' ? 'realistic mix' : 'positive'}) for "${product.title}"${dupNote}`,
    user_id: req.user?.user_id || null, initiator: 'user',
  });

  return res.status(200).json({ inserted, skipped_duplicates });
}
