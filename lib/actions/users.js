...
export async function update_user(req, res) {
  ...
  // Last-admin protection (P1-17, AUDIT-2026-08): the count-check + write used
  // to be a separate SELECT-then-UPDATE with no lock between them — two
  // concurrent demote/deactivate calls against 2 different admins could both
  // pass the ">1 active admin" check before either write landed, leaving zero
  // active admins. safe_update_user (sql/add-safe-admin-update-fn.sql) does the
  // check + write atomically inside one transaction with FOR UPDATE row locks,
  // so a concurrent call against a different admin serializes behind this one.
  const { data, error } = await supabase
    .rpc('safe_update_user', { p_user_id: user_id, p_updates: updates })
    .single();
  ...
  if (error) {
    ...
    console.error('[update_user] rpc failed:', error);
...
export async function delete_user(req, res) {
  ...
  // Last-admin protection (P1-17, AUDIT-2026-08) — see update_user for the
  // TOCTOU rationale. safe_delete_user checks + deletes atomically under a
  // FOR UPDATE lock on the target + all other active admins.
  const { data, error } = await supabase.rpc('safe_delete_user', { p_user_id: user_id });
  if (error) {
    if (error.message?.includes('last_active_admin')) {
    ...
    console.error('[delete_user] rpc failed:', error);
    return res.status(500).json({ error: 'failed to delete user' });
  }
  if (!data) {
    console.error('[delete_user] rpc returned falsy without an error:', data);
    return res.status(500).json({ error: 'failed to delete user' });
  }
...
export async function generate_api_token(req, res) {
  ...
  // P2 fix (AUDIT-2026-08, I-2 tech debt): api_token was stored in plaintext.
  // Store only the SHA-256 hash from here on — the plaintext is returned once
  // in this response and never persisted. See sql/add-users-api-token-hash.sql
  // (dual-mode backfill) and lib/auth.js's verifyApiToken (hash lookup +
  // plaintext fallback for tokens generated before this deploy).
  const api_token_hash = createHash('sha256').update(api_token).digest('hex');

  const { data, error } = await supabase
    .from('users')
    .update({ api_token: null, api_token_hash })
    .eq('id', user_id)
    .select('id, username')
    .single();
