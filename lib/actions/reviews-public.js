...
//
// CORS note for theme/widget authors (P1-A, AUDIT-2026-08-B): this action, plus
// vote_review_helpful and review_helpful_counts below, are gated by per-store CORS
// (lib/storefront-cors.js). The browser's CORS preflight resolves the store from your
// Origin header automatically — no widget change required. As a robustness belt, the
// widget MAY also append `?store_id=<this store's TC UUID>` to the fetch URL (e.g.
// `/api/system?action=submit_review_public&store_id=...`); a preflight carries the same
// query string as the real request, so this resolves the store without any DB lookup at
// all. Not required, but recommended — see Docs/RUNBOOK-new-store.md section 4.
