...
export async function profit_summary(req, res) {
  ...
  // Business-sensitive margin data — non-admins must scope to a store they have access
  // to; an unscoped call would otherwise return combined P&L across every store.
  ...
  // Get all products with COGS + variant COGS (filtered by store)
  let productsQuery = supabase.from('products').select('title, cogs, variant_cogs');
  if (storeId) productsQuery = productsQuery.eq('store_id', storeId);
  const { data: products } = await productsQuery;
  const cogsMap = {};
  const variantCogsMap = {};
  (products || []).forEach((p) => {
    if (p.cogs) cogsMap[p.title] = parseFloat(p.cogs);
    if (p.variant_cogs && typeof p.variant_cogs === 'object') variantCogsMap[p.title] = p.variant_cogs;
  });
  const missingCogs = (products || []).filter((p) => !p.cogs).length;

  // Get Meta adspend from performance table (filtered by store)
  let metaPerfQuery = supabase.from('performance').select('date, spend').gte('date', sinceStr);
  if (storeId) metaPerfQuery = metaPerfQuery.eq('store_id', storeId);
  const { data: metaPerf } = await metaPerfQuery;

  // Get manual adspend (filtered by store)
  let manualSpendQuery = supabase.from('manual_adspend').select('*').gte('date', sinceStr);
  if (storeId) manualSpendQuery = manualSpendQuery.eq('store_id', storeId);
  const { data: manualSpend } = await manualSpendQuery;
  ...
  for (const order of filteredOrders) {
    ...
    // COGS from line items — use variant-level COGS if available, else product-level
    for (const item of order.items || []) {
      let unitCost = cogsMap[item.title] || 0;
      const vc = variantCogsMap[item.title];
      if (vc && item.variant_title) {
        // Try exact match on variant option (e.g. "Black" in "Black / M")
        for (const [optVal, cost] of Object.entries(vc)) {
          if (item.variant_title.includes(optVal)) { unitCost = parseFloat(cost); break; }
        }
      }
