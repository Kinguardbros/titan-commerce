...
export async function bulk_price(req, res) {
  ...
  await supabase.from('pipeline_log').insert({ agent: 'PRICING', level: 'info', store_id, message: `Bulk updated ${updated} variants across ${product_shopify_ids.length} products to $${new_price}`, user_id: req.user?.user_id || null, initiator: 'user' });
