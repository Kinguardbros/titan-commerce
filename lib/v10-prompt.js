...
EXPOSURE:
The SUBJECT (model + garment) is the brightest, most exposed element in the frame — exposed for clean, crisp readability with bright open skin tone, fully lit garment fabric showing every construction detail, and rich highlight detail. The BACKGROUND (sand, sky, ocean) is exposed SLIGHTLY DARKER than the subject — still clearly visible with full detail (NOT vaporised to white, NOT a foggy haze, NOT a gloomy dark grey, NOT silhouetted), but about 1 stop dimmer than the subject so the model and garment visually POP off the background. The exposure relationship: subject = bright and primary, background = visible but recedes. The subject is never overexposed, never blown out, never washed out. Black fabric reads as a rich dark grey-black with the ribbed texture / pleating / seams clearly visible — NOT crushed to a flat black silhouette.
...
- Subject and background exposed identically with no separation — subject should be visibly brighter than the background
