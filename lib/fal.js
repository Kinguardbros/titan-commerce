 ...
 * `resolution` ("1K"|"2K"|"4K") only applies to Nano Banana models; others ignore it.
 */
function buildFalBody(model, prompt, imageUrl, numImages, aspectRatio, resolution) {
  ...
  // Flux Schnell: uses image_size presets (NOT aspect_ratio)
  // Valid: square_hd, square, portrait_4_3, portrait_16_9, landscape_4_3, landscape_16_9
  if (model.includes('flux/schnell') || model.includes('flux-pro/v1') || (model.includes('flux') && !model.includes('edit') && !model.includes('kontext') && !model.includes('flux-2'))) {
    const SCHNELL_RATIO_MAP = {
      '1:1': 'square_hd',
      '3:4': 'portrait_4_3',
      '4:3': 'landscape_4_3',
      '9:16': 'portrait_16_9',
    ...
    const imageSize = SCHNELL_RATIO_MAP[ratio] || 'square_hd';
    return { prompt, num_images: numImages, image_size: imageSize };
  }

  // Default: nano-banana-2/edit and nano-banana-pro/edit (image_urls list).
  // Both default to "1K" — too soft for product photos. Caller picks the resolution;
  // fall back to "2K" if not provided.
 ...
 */
export async function generateFal({ model = 'fal-ai/nano-banana-2/edit', prompt, imageUrl, numImages = 1, aspectRatio = '1:1', resolution }) {
  if (!FAL_KEY) throw new Error('FAL_KEY not configured');

  const body = buildFalBody(model, prompt, imageUrl, numImages, aspectRatio, resolution);
  console.log('[fal] Submit to:', model, 'images:', Array.isArray(imageUrl) ? imageUrl.length : (imageUrl ? 1 : 0), 'resolution:', body.resolution || 'n/a');
  ...
  const { request_id, status_url } = responseData;
  if (!request_id) throw new Error('No request_id from fal.ai');

  // fal.ai returns the correct status_url in the submit response — derive pollBase from it.
  // For e.g. fal-ai/ideogram/v3/replace-background it returns status_url = .../fal-ai/ideogram/requests/{id}/status,
  // so pollBase = "fal-ai/ideogram" (parent of /requests/{id}/status). Deterministic, no guessing.
  const pollBase = derivePollBase(status_url, model);
  return pollFalResult(pollBase, request_id);
}

// Strip /requests/{id}/status suffix from a fal.ai status_url and the queue.fal.run host
// to get the pollBase model path. Falls back to the model itself if status_url is absent.
function derivePollBase(statusUrl, model) {
  if (!statusUrl) return model;
  return statusUrl
    .replace(/^https?:\/\/[^/]+\//, '')
    .replace(/\/requests\/[^/]+\/status\/?$/, '');
}
 ...
 */
export async function submitFalJob({ model = 'fal-ai/nano-banana-2/edit', prompt, imageUrl, numImages = 1, aspectRatio = '1:1', resolution }) {
  if (!FAL_KEY) throw new Error('FAL_KEY not configured');

  const body = buildFalBody(model, prompt, imageUrl, numImages, aspectRatio, resolution);
  console.log('[fal] Submit job to:', model, 'resolution:', body.resolution || 'n/a');
  const res = await fetch(`${FAL_BASE}/${model}`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json', 'Authorization': `Key ${FAL_KEY}` },
    body: JSON.stringify(body),
  ...
  // Derive pollBase from the status_url fal.ai returned — deterministic for every model.
  const pollBase = derivePollBase(data.status_url, model);
