...
export async function generateFal({ model = 'fal-ai/nano-banana-2/edit', prompt, imageUrl, numImages = 1, aspectRatio = '1:1', resolution }) {
  ...
  // Strip trailing path segments for polling — only for models that need it.
  // Nano Banana (fal-ai/nano-banana-*/edit) and FLUX (fal-ai/flux-pro/kontext, fal-ai/flux/schnell)
  // are polled at the parent path. Ideogram v3 (replace-background, edit) is polled at the FULL
  // model path — don't strip. Default: keep the full path.
  const pollBase = /^fal-ai\/(nano-banana|flux)/.test(model)
    ? model.replace(/\/(edit|kontext|schnell)$/, '')
    : model;
...
export async function submitFalJob({ model = 'fal-ai/nano-banana-2/edit', prompt, imageUrl, numImages = 1, aspectRatio = '1:1', resolution }) {
  ...
  // Same rules as generateFal above: only Nano Banana and FLUX strip the suffix; Ideogram
  // (replace-background, edit) polls at the full model path.
  const pollBase = /^fal-ai\/(nano-banana|flux)/.test(model)
    ? model.replace(/\/(edit|kontext|schnell)$/, '')
    : model;
