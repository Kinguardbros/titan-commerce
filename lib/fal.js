 ...
 */
function buildFalBody(model, prompt, imageUrl, numImages) {
  ...
  if (model.includes('ideogram') && model.includes('replace-background')) {
    ...
    return { image_url: images[0], prompt, num_images: numImages };
  }
...
export async function generateFal({ model = 'fal-ai/nano-banana-2/edit', prompt, imageUrl, numImages = 1 }) {
  ...
  const body = buildFalBody(model, prompt, imageUrl, numImages);
